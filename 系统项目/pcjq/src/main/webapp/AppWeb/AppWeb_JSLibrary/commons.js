 ...
 * @returns
 */
function trimString(str){ //删除左右两端的空格
    if(str!=null&&str!='')
...
function checkName(name) {
	...
	if (name.length < 2 || name.length > 20) {
		show_tips("姓名长度有误，请重新输入");
		return false;
	}

	if (!Validater.isName(name)) {
		show_tips("姓名格式必须为汉字、英文或点号");
		return false;
	}
...
/* 银行卡格式化 */
function checkBankNumberLength(temp) {
...
function replaceStr1(str,num1,num2){
	...
	var strTemp = str.substr(num1,num2);
	var strSuff = str.substr((num1+rep.length),num2);
	var strRes = strTemp.replace(strTemp,rep);
	return strPre+strRes+strSuff;
}
/**
...
function gotoRealName(){
	show_tips("请您完成实名鉴权");
	setTimeout("goToURL('/AppService/business/bank/realName.shtml')", 2000);
}

/*跳转到实名鉴权页面*/
function gotoRealNamePage(pageId,eventId){
	show_tips("请您完成实名鉴权");
	setTimeout("goToURL('/AppService/business/bank/realName.shtml?pageSourceId="+pageId+"&eventId="+eventId+"')", 2000);
}

/*跳转到设置安全码页面*/
function gotoSetTPassword(){
	show_tips("请先去设置安全码");
	setTimeout("goToURL('/AppService/business/account/setTPassword.shtml')", 2000);
}
/**
 ...
 * 根据消息标题模糊查询
 * @param title
 * @returns {___anonymous_userMsgOtherData}
 */
function queryMsgLikeTitle(title){
	$.ajax({
        async: false,
        url: "/AppService/business/queryUserMsgLikeTitle.xhtml",
        dataType: "json",
        cache: true,
        data: {
            'title' : title
        ...
        success: function(data) {
        	if(!!data){
        		userMsgOtherData = data;
        	};
        }
    });
	return userMsgOtherData;
}
				...
				if(riskLevel == 0 || riskLevel == '0'){
					...
					$("#noOperation .risk .btn").click(function(){				
						...
						if(pageSourceId != '' && pageSourceId != null && pageSourceId != undefined){
							url = "/AppService/business/fund/fundList.shtml?pageSourceId="+pageSourceId+"&eventId="+eventId;
						} else {
							url = "/AppService/business/fund/fundList.shtml?pageSourceId="+pageId+"&eventId="+eventId;
						}
				...
				if(resultData){
				    ...
				    if(dataDiff >= 365){
						...
						$("#noOperation .anewRisk .btn").click(function(){
							...
							var url = "";
							if(pageSourceId != '' && pageSourceId != null && pageSourceId != undefined){
								url = "/AppService/applicationGroups.shtml?mainCatId=MY_ACCOUNT&thirdCatId=ACCOUNT_INFO&operation=toRiskLevel&pageSourceId="+pageSourceId+"&eventId="+eventId;
							} else {
								url = "/AppService/applicationGroups.shtml?mainCatId=MY_ACCOUNT&thirdCatId=ACCOUNT_INFO&operation=toRiskLevel&pageSourceId="+pageId+"&eventId="+eventId;
							}
							$("#noOperation").hide();
							$("#noOperation .risk").hide();
							$("#noOperation .anewRisk").hide();
...
function queryUserinfoWithUserType(){
...
function pageEventData(registmark,type){
	...
	$.ajax({
		async : false,
		url : "/AppService/business/insertUserOperateLog.xhtml",
 ...
 * @returns
 */
function queryParamComm(paramType,paramKey,pmValueOne){
    var data = null;
    $.ajax({
        async: !1,
