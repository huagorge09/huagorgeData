...
$(document).ready(function () {
    ...
    $("#proID").html(getUrlParameter("fundId"))
    /* 产品详情底部问答 b */
...
//							$("#orderDetail .head-righ-con div.order-info p:eq(1)").html("预期收益："+benefit);/* 预期收益 */
                    ...
                    if (typeId != null && typeId == "0100") { /* 固定收益 */
                        $("#top_left_01").show();
                        $("#top_left_02,#top_left_03,#top_left_04,#netval").remove();
                        ...
                        $("#top_left_04").show();
                        $("#top_left_01,#top_left_02,#top_left_03,#netval").remove();
                        ...
                        $("#top_left_02").show();
                        $("#top_left_01,#top_left_03,#top_left_04,#netval").remove();
...
function queryEstimateByFundId(dateTime){
	...
	$.ajax({
		async:true,
		url : "/AppService/setUp/queryInWhiteListFundNetValueByImgTable.xhtml",
		data : {
			fundId:fundId,
			dateTime:dateTime,
		},
...
function queryEstimateByFundIdByPage(pages){
	...
	$.ajax({
		async:true,
		url : "/AppService/setUp/queryInWhiteListFundNetValue.xhtml",
		data : {
			fundId:fundId,
			page:page
		},
...
function checkUserLogin() {
    var querySource = getUrlParameter("querySource");
    if(querySource=="personal"){ //个人查询净值
        checkIsLogin()
    }else{   //企业用户查询净值
        checkCompanyUser()
    }
    ...
    $.ajax({
        ...
        success : function (data) {
            if(data.returnCode != null && data.returnCode == "0000"){
                $("#loginYesPublic").show();
                $("#login_userinfo_public").html(data.fundAccoEncry).prop("href","/company/companyList.shtml");
                $(".My-center a").prop("href","/company/companyList.shtml");
                $("#loginYes").hide();
                $("#loginNo").hide();
