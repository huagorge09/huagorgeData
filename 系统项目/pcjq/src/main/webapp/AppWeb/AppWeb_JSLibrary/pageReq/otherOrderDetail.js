...
function queryTradeInfoByTradeNo(){
    ...
    $.ajax({
    	async:false,
		url:"/AppService/business/queryOtherDetailOrder.xhtml",
		type:"post",
		dataType:'json',
		data:{
			serialno:serialno,
		...
		success:function(data, textStatus){
			...
			if(data.agentFundDto != null){
				...
				if(fundInfoDto != null){
					...
					var profit = fundInfoDto.profit;/* 产品预期收益率*/
...
//					var sellPercent = numMulti(numDiv((scale-displayLimit),scale),100).toFixed(0);/* 产品额度(销售进度百分比)*/
					...
					var benefit = dto.profit;/* 预期收益 */
					var shareAmt = dto.shareAmt;/* 分配金额 */
					...
					if(dto.applySt!= null && dto.applySt == "G"){/* 存续中 */
						...
						$("#orderDetail .head-righ-con div.order-info p:eq(0)").html("买入金额："+subamt);/* 订单金额 */
...
//							$("#orderDetail .head-righ-con div.order-info p:eq(1)").html("预期收益："+benefit);/* 预期收益 */
							...
							$("#orderDetail .head-righ-con div.order-info p:eq(1)").html("持仓盈亏："+benefit);/* 持仓盈亏 */
						}else if(fundInfoDto.typeId != null && fundInfoDto.typeId == "0300"){/*浮动收益产品*/
							$("#orderDetail .head-righ-con div.order-info p:eq(1)").html("已分配金额："+shareAmt);/* 预期收益 */
						}else{/*默认浮动收益产品展示*/
							$("#orderDetail .head-righ-con div.order-info p:eq(1)").html("已分配金额："+shareAmt);/* 预期收益 */
						}
						$("#orderDetail .head-righ-con div.check-cancel").remove();/*去掉按钮--查看汇款信息和取消订单的按钮*/
						$("#orderDetail .head-righ-con p.point").html("产品处于存续期，预计将于<em>"+formatDate1(maturityDate)+"</em>到期");/*订单提示语*/
        			}else if(dto.applySt != null && dto.applySt == "Z"){/* 已到期 */
						$("#orderDetail h1.floatincome").html("已到期...").addClass("timeout");
						
						$("#orderDetail .head-righ-con div.order-info p:eq(0)").html("买入金额："+subamt+"</em>");/* 订单金额 */
						if(fundInfoDto.typeId != null && fundInfoDto.typeId == "0100"){/*固定收益产品*/
							//$("#orderDetail .head-righ-con div.order-info p:eq(1)").html("投资收益："+benefit);/* 预期收益 */
							...
							$("#orderDetail .head-righ-con div.order-info p:eq(1)").html("总分配金额："+shareAmt);/* 预期收益 */
						}else{/*默认浮动收益产品展示*/
							$("#orderDetail .head-righ-con div.order-info p:eq(1)").html("总分配金额："+shareAmt);/* 预期收益 */
						}
						$("#orderDetail .head-righ-con div.check-cancel").remove();/*去掉按钮--查看汇款信息和取消订单的按钮*/
						$("#orderDetail .head-righ-con p.point").html("产品已到期，预计将于5个工作日内完成收益分配");/*订单提示语*/
        			}
					/*-------------------- 订单详情展示  E --------------------*/
					...
					if (typeId != null && typeId == "0100") { /* 固定收益 */
						...
						if (dto.applySt!= null && dto.applySt == "G") {/* 存续中 */
							...
							$("#top_left_04 #netval_profit").html("<em>" + numMulti((parseFloat(fundInfoDto.latestNewValue) - dto.confirmNav), 100).toFixed(2) + "</em>%");/*累计收益率*/
							$("#top_left_04 #netval_text").html("<em><b>" + $("#adname").text() + "最新净值日期为<i>" + fundInfoDto.lastNavDate + "</i></b></em>");/*描述*/
							$("#top_left_04 #maxNetValue").html(parseFloat(fundInfoDto.maxNav).toFixed(4));/*历史最高净值*/
						} else if (dto.applySt!= null && dto.applySt == "Z") {/* 已到期 */
						...
						if (dto.applySt!= null && dto.applySt == "G") {/* 存续中 */
							...
							$("#top_left_03 #netval_profit").html("<em>" + numMulti((parseFloat(fundInfoDto.latestNewValue) - dto.confirmNav), 100).toFixed(2) + "</em>%");/*累计收益率*/
							...
							// queryEstimateByFundIdByPage("1");/* 查询产品净值 */
						} else if (dto.applySt!= null && dto.applySt == "Z") {/* 已到期 */
