...
/**
 * 描述: 版权: Copyright (c) 2005 公司: 思迪科技 作者: 易庆锋 版本: 1.0 创建日期: 2006-10-4 创建时间:
 * 17:42:07
 */
public final class StringHelper
{
	...
	/**
	 * 作者: chen 时间: 2008-6-4 下午06:08:19 描述: 检查用户
	 * @param username
	 * @return
	 */
	public static boolean checkstr(String username)
	{
		...
		String numberStr = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789_";
...
//	        	 CharBuffer charBuffer = charset.decode(); 
	...
	/**
	 * 作者: chen 时间: 2008-6-4 下午06:46:02 描述: 检查Email
	 * 
	 * @param str
	 * @return
	 */
	public static boolean chenckEmail(String str)
	{
		...
		if (str != null && str.length() > 0)
		{
			...
			{
				// System.out.println("对不起，您输入的email地址1:"+chen+"不正确！");
				return false;
			}
			if (chen.indexOf("@.") > 1)
			{
				// System.out.println("对不起，您输入的email地址2:"+chen+"不正确！");
				return false;
			}
			if (tempd == -1 || (len - tempd) < 2)
			{
				// System.out.println("对不起，您输入的email地址3:"+chen+"不正确！");
				return false;
			}
		}
		return true;
	...
	public static String getFilterStr(String str)
	{
		...
		if (filterStr != null && filterStr.length() > 0)
		{
			...
			filterStr = replace(filterStr, "'", "&quot");
			filterStr = replace(filterStr, "+", "\\+");
			filterStr = replace(filterStr, " ", "&ensp;");
			filterStr = replace(filterStr, "=", "\\=");
			filterStr = replace(filterStr, "(", "\\(");
			filterStr = replace(filterStr, ")", "\\)");
		}
