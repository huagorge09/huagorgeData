...
import com.cmwa.ec.webapp.dto.BaseDto;
...
public class StringUtils {
    ...
    /**
     * 拼消息报文到前置机
     * @param msgtype template:模板消息,service：客服消息
     * @param msgcontent
     * @return
     */
	...
	/**
	 * 将传入的字符串加密处理
	 * 	ex:将银行卡号加密处理，只保留前4为和末4位，其他以‘*’代替
	 * @param no		需要处理的字符串
	 * @param psw		加密的字符，即上面‘ex’中的‘*’
	 * @param length1	字符串前面保留的长度
	 * @param length2	字符串末尾保留的长度
	 * @return
	 * 			String
	 * @author maj
	 */
	public static String replaceToNo(String no,String psw,Integer length1,Integer length2){
		String temp = "";
		String val = "";
		for (int i = 0; i < no.length()-(length1+length2); i++) {
			temp = temp + psw;
		}
		val = no.substring(0, length1) + temp + no.substring(no.length() - length2, no.length());
