...
package com.cmwa.ec.webapp.util.upload.jspsmart;
...
public class SmartUpload{
  ...
  public void upload() throws SmartUploadException,IOException,
    ...
    do{
      ...
      if(isFile){
        com.cmwa.ec.webapp.util.upload.jspsmart.File newFile = new com.cmwa.ec.webapp.util.upload.jspsmart.File();
  ...
  // add by bruce lau for multiple file download
	public void downloadZipFile(String data, String destFileName)
		...
		int BUFSIZE = 1024 * 8;
		String name = "";
		if (destFileName != null && destFileName.length() > 4) {
			name = destFileName.substring(0, destFileName.length() - 4);
		}
		ZipOutputStream zos = new ZipOutputStream(m_response.getOutputStream());
		String[] strs = null;
		if (data != null) {
			strs = data.split("#");
		}
		if (strs != null && strs.length != 0) {
			...
			for (int i = 0; i < strs.length; i++) {
				File f = new File(name + "_" + (i + 1) + ".txt");
				BufferedWriter output = new BufferedWriter(new FileWriter(f));
				output.write(strs[i]);
				output.close();
				//File f1 = new File(name + "_" + (i + 1) + ".txt");
