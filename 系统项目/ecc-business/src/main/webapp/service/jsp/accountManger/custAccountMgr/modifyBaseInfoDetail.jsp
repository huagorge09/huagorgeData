...
<body class="fixed-nav gray-bg">
	<div class="modal-content" style="border: 0px;box-shadow: 0 0px 0px;">
		...
		<input id="preRisklevel" value='${dto.custrisklevl }' type="hidden">
		
		<div class="page-header" style="margin: 0;">
			<h4 class="page-title">基本资料修改</h4>
						...
						<tr>
							...
							<td class="white-bg">
								...
								<div style="display: none;" class="orgContactrightDiv">
									<div class="col-sm-11 form-inner">
										<select name="orgContactright" id="ocontactgrant" class='form-control orgContactright'>
											<c:forEach var="item" items="${bokerprivArray}">
							...
							<td class="white-bg">
								<div class="col-sm-11 form-inner">
									<select name="orgHoldingidtp" id="orgHoldingidtp" class='form-control select2_width'>
										<c:forEach var="item" items="${seatidtpArray}">
											<c:if test="${dto.holdingidtp != '' and dto.holdingidtp != null}">
												<option value="${item.PMCO}" ${item.PMCO == dto.holdingidtp ?"selected": "" }>${item.PMCO}&nbsp;&nbsp;${ item.PMNM }</option>
											</c:if>
											<c:if test="${dto.holdingidtp == '' or dto.holdingidtp == null}">
												<option value="${item.PMCO}" ${item.PMCO == '11' ?"selected": "" }>${item.PMCO}&nbsp;&nbsp;${ item.PMNM }</option>
											</c:if>
										</c:forEach>
									</select>
...
<script type="text/javascript" src="<%=context%>web/js/accountManger/custAccountMgr/modifyBaseInfoDetail.js?v="<%=dateStr%>></script>
</body>	
</html>