	            ...
	            <span class="form-input">
	            	<select name="selCheckStatus" id="selCheckStatus" style="width: 250px;" class='form-control select2_init'>
						...
						<c:forEach var="item" items="${chkflgArray}">
							<option value="${item.PMCO}" ${item.PMCO == 'N' ? "selected" : "" }>${item.PMCO}&nbsp;&nbsp;${ item.PMNM }</option>
						</c:forEach>
					</select>
	            </span>
	         </div>
...
<script type="text/javascript" src="<%=context%>web/js/accountManger/custAccountMgr/checkAccountInfoList.js?v="<%=dateStr%>"></script>
</body>
</html>