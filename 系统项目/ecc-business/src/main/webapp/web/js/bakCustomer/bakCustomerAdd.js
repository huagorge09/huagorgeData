...
var contflag=0;
var flag = 0;
//用于动态设置证件有效期的长期有效功能，由于ID改变，原取值方式也需要修改
...
if(!!contnm && !!contidno){
 ...
 trstr += $(".orgContactrightDiv").clone(true).find("#ocontactgrant").attr("id","oocontactgrant").val(contgrant).end().html();
 
 trstr +="</td><td>经办人姓名：</td><td class='white-bg'>";
 trstr += $("#continfo>tr:eq(0)").find('td').eq(3).clone(true).find("#orgContnm").attr("id","oocontact").val(contnm).end().html();
 trstr +="</td></tr>";               
 //第三行
 trstr +="<tr>";
 trstr +="<td>经办人国籍：</td><td class='orgContnationTd white-bg'>"+$(".orgContnationDiv").clone(true).find(".orgContnation").attr("id","oocontactnation").val(contnation).end().html()+"</td>";
 trstr +="<td>经办人证件类型：</td>" +
 		   "<td class='orgContidtpTd white-bg'>"+$(".orgContidtpDiv").clone(true).find(".orgContidtp").attr("id","oocontidtp").val(contidtp).end().html()+"</td>";
 trstr +="</tr>";
 //第四行
 trstr +="<tr>";
 trstr +="<td>经办人证件号码：</td>";
 ...
 trstr +="<td>经办人办公电话：</td>"
 trstr +="<td class='white-bg'><div class='col-sm-11 form-inner'>" +
	   "<input name='oocontphone' style='float: left;' class='form-control' id='oocontphone"+"' type='text'>";
 trstr +='<label style="line-height: 34px;">';
 trstr +="<td>经办人传真号码：</td>"
 trstr +="<td class='white-bg'><div class='col-sm-11 form-inner'>" +
	   "<input name='oocontfax' style='float: left;' class='form-control' id='oocontfax"+"' type='text'>";
 trstr +='<label style="line-height: 34px;">';
/* trstr +=$("#continfo>tr:eq(3)").clone(true).find("#orgContphone").attr("id","oocontphone").val(conttel).end().find("#orgContfax").attr("id","oocontfax").val(contfax).end().html();*/
 trstr +="</tr>"; 
 //第六行
 trstr +="<tr>";
 trstr +=$("#continfo>tr:eq(4)").clone(true).find("#orgContmobile").attr("id","oocontmobile").val(contmobile).end().find("#orgContemail").attr("id","oocontemail").val(contemail).end().html();
 trstr +="</tr>";
 //第七行
 trstr +="<tr>";
 trstr +=$("#continfo>tr:eq(5)").clone(true).find("#orgContAddr").attr("id","oocontAddr").val(contAddr).end().find("#orgContPostcode").attr("id","oocontPostcode").val(contPostcode).end().html();
 trstr +="</tr>";
 //第八行
...
function dosubmit()
{
	...
	$.ajax({
		url: PRIMARY_PATH+"/addBakCustomer.xhtml",  
		dataType: "json",
		type: "POST",
		data:  {
