...
function fillBankInfo(object)
{
   ...
   else
   {
      ...
      $("#openBank").empty().append(dataObj.bankNo);
      $("#openName").empty().append(dataObj.openName);
      $("#accoName").empty().append(dataObj.bankAcNm);
      $("#bankAcco").empty().append(dataObj.bankAcco);
      $("#openName").empty().append(dataObj.openName);
      $("#tradeacco").val(dataObj.tradeAcco);
      $("#hidtano").val(dataObj.tano);
   }      
}
...
function fillData(object)
{
   ...
   if(Number(tradeacco.length)>0)
   {
      ...
      {
         $("#seltradeacco").append('<option value="'+JsonToStr(tradeacco[i])+'">'+tradeacco[i].tano+"  "+tradeacco[i].tradeAcco+"</option>");
      }
   }
   $("#seltradeacco").change();
...
function deleteTradeAcco() {
	...
	if (qParam.tradeacco == '') {
		ctools.alert("请选择交易账号！","","error");
		return false;
	}
	var tano = $("#hidtano").val();
	if (tano == '') {
	...
	$.ajax({
		url: ACCOUNT_PATH+"deleteTradeAccount.do",  
		dataType: "json",
		type: "POST",
		data:  {
