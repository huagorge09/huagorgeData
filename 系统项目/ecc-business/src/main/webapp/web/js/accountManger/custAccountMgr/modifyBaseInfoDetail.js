...
function getOrgInfo()
{
   ...
   var oidnoStr = "";  
   var oidvalidateStr = "";
   $("#oidinfo select[id='ooidtp']").each(function(data)
   {
   ...
   $("#oidinfo input[id='ooidno']").each(function(data)
   {
      if($(this).val()=="" || $(this).val() == null)
      {
         oidnoStr += "$,";
   ...
   $("#oidinfo input[name='ooidvalidate']").each(function(data)
   {
      if($(this).val()=="" || $(this).val() == null)
      {
         oidvalidateStr += "$,";
...
function dosubmit()
{
   ...
   $.ajax({
		url: ACCOUNT_PATH+"modifyAccount.do",  
		dataType: "json",
		type: "POST",
		data:  {
