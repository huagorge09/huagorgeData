...
function initGrid() {
	parameterList
			.jqGrid({
				url :  PRIMARY_PATH + '/parameter.xhtml',
				mtype:"GET",
				...
				loadComplete : function(data) {
					if (data.ResultCode == '8000') {
						swal({
							...
							if(isConfirm){
								window.location.href = BASE_PATH + 'jsp/login.jsp';
							}
						})
					}
				},
				...
				gridComplete : function() {
					...
					for (var i = 0; i < ids.length; i++) {
						...
						var be = '<a date-startupid="'
								+ id
								+ '" href="#" class="btn btn-link btn-jqgrid" title="点击展开特殊事项列表" data-toggle="modal" onclick="expandSpecialMatter(\''
								+ id
