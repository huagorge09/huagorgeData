...
function doAudit(){
	if(isAudit == 'N'){
		var html = '<form action="" id="checkForm">';
			html+='<table class="table table-bordered" style="width: 80%;margin: 0 auto;border: 0px;">';
			html+='<colgroup><col width="20%"><col width="40%"></colgroup>';
			html+='<tbody style="border: 0;font-size: 16px;">';
			html+='<tr><td style="border: 0;display: block;margin-top: 10px;">主管工号：</td>';
			html+='<td class="white-bg form-inner" style="border: 0;">';
			html+='<input type="text" placeholder="主管工号" class="form-control" id="managerId" name="managerId" style="display:  block;margin: 0;font-size: 16px;">';
			html+='</td></tr>';
			html+='<tr><td style="border: 0;display: block;margin-top: 10px;">主管密码：</td>';
			html+='<td class="white-bg form-inner" style="border: 0;">';
			html+='<input type="password" placeholder="主管密码" class="form-control" id="managerPwd" name="managerPwd" style="display: block;margin: 0;font-size: 16px;">';
			html+='</td></tr>';
			html+='</tbody>';
			html+='</table>';
			html+='</form>';
		ctools.confirm({
			...
			type : null,
			showCancelButton : true,// 显示关闭按钮
			confirmButtonColor : "rgb(252, 104, 33)",// 确定按钮的颜色
			confirmButtonText : "确定",// 确定按钮的文本
			cancelButtonText : "取消",// 取消按钮的文本
			closeOnConfirm : false,// 确定按钮是否关闭模态框
			closeOnCancel : true// 取消按钮式否关闭模态框
		},function(isConfirm){
			if(isConfirm && $("#checkForm").valid()){
				$("#audit").text("取消授权");
				$(".checkno").text($("#managerId").val());
				$(".checkpwd").text("******");
				$("#checkno").val($("#managerId").val());
				$("#checkpwd").val($("#managerPwd").val());
				isAudit = "Y";
		...
		ctools.confirm({title : "是否确认取消授权？",text : "操作确认"},
		function(isConfirm){
			if(isConfirm){
				$("#audit").text("授权");
				isAudit = "N";
...
$(function(){
	$(document).ajaxComplete(function(event, request, settings) {
		var ajaxRedirectFlag = request.getResponseHeader("ajaxRedirectFlag");
		var sendredirecturl = request.getResponseHeader("sendredirecturl");
		if(!!ajaxRedirectFlag){
			if("302" == ajaxRedirectFlag && !!sendredirecturl ){
