(function(factory) {
    ...
    Upload.prototype = {
        ...
        init: function(config) {
            ...
            this.doneCall = config.doneCall || null;
            this.failCall = config.failCall || null;
            //日期转换函数
            ...
            if (typeof config.donePreview == 'string') {
                ...
                this.donePreview = function(f) {
                    return  $('<tr class="uploaded" attId="{{id}}" >\
	                            <td><a href="/cbp/product/archive/attach/downloadAttach.do?attId={{id}}" class="showunderline">{{name}}</a></td>\
	                            <td>{{date}}</td>\
	                            <td>已上传</td>\
            ...
            //点击上传按钮允许自定义校验事件，校验通过才弹出文件上传框
            if ("function" === typeof config.previewValidateFunc){
            	this.previewValidateFunc=config.previewValidateFunc;
        	}else{
				this.previewValidateFunc=function(){return true;};
			}
            if ("string" === typeof config.fileInputName) {
            	this.fileInputName= config.fileInputName;
			}else{
        ...
        bindEvent: function() {
            ...
            this.$uploadButton.on('click', function() {
            	var isValidate=self.previewValidateFunc.call();//点击上传按钮允许自定义校验事件，校验通过才弹出文件上传框
            	if (isValidate) {
            ...
            this.$container.on('click', '.f-cancel', function() {
                ...
                $(this).parents('tr[class=uploading]').remove();
            })
        },
        //创建input[type]
        ...
        detectFile: function(f) {
            ...
            var flag = true;
            if (!f.type) {
				var suffix = (f.name.toLowerCase()).substring(f.name.lastIndexOf('.')+1);
				if ('zip'.indexOf(suffix) == -1) {
					 self.showError({ msg: self.config.typeMessage || '文件类型不对', file: f});
		             flag = false;
				}
			}else if (self.config.type && self.config.type.indexOf(f.type) == -1) {
                self.showError({ msg: self.config.typeMessage || '文件类型不对', file: f});
                flag = false;
            } else if (self.config.size && f.size > self.config.size) {
//            	alert(f.size);
            	//文件大小是字节为单位，需要保持跟系统文件系统字节数配置一致，请准确配置以保证提示语正确
                self.showError({ msg: '上传的文件超出大小限制！请上传大小为'+parseInt(self.config.size/(1024*1024))+'M以下的文件！', file: f});
                flag = false;
            } else if (self.config.length && self.config.length <= self.$container.find('.uploaded, .uploading').length) {
                self.showError({ msg: '已超出最大可上传文件数量:'+self.config.length, file: f});
                flag = false;
            }
            return flag;
        },
        ...
        upload: function(f, $p) {
            var self = this;
            var fd = new FormData();
            fd.append(self.fileInputName, f);
            ...
            var _ajax = $.ajax({
                ...
                .done(function(data) {
                    //处理预览的DOM
                    $p.replaceWith(self.previewHandle(data, self.donePreview));
                    
                    if (self.doneCall && typeof self.doneCall == 'function') {
                    	self.doneCall(data);
                    }
                })
                .fail(function(xhr, status) {
                    if (status != 'abort') {
                        ...
                        if (self.failCall && typeof self.failCall == 'function') {
                        	self.failCall(xhr, status);
                        }
                    }
                })
