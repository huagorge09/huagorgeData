...
function validateForm(){
	// 在键盘按下并释放及提交后验证提交表单
	queryValidate = $("#switchSelfQuery").validate({
		rules: {
			tradeacco: {
				checkTradeacco: true
			},
...
jQuery.validator.methods["checkSeatno"]=(function(value,element){
	var oTradeacco = $("#oTradeacco").val();
	var success=false;
	var queryflag = $("#queryflag").val();
	if((oTradeacco == "" || oTradeacco == null) && queryflag == "Y"){
		success = false
	}else{
		success = true
	}
	...
	ctools.confirm({text:title},function(isConfirm){
		if(isConfirm){
			$.ajax({
				...
				async: false, 
				dataType : "json",
				data: $('#switchSelf').serialize(),// 你的formid
