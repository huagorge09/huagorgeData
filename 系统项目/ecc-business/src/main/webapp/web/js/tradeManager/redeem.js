...
function capMoneyNoCheck(xxje,amtNM) {
	...
	{
		return "<font color=red>您输入的份额格式不正确！</font>"; 
	...
	cap = cap.replace( /零$/, "整");
	cap = cap.replace( /角|分/g, "");
	
	if (cap == "整") cap = "零元整";
	
	if(cap.indexOf("元整") > -1){
		cap = cap.replace( /元整/g, " 份");
	}else{
		cap = cap.replace( /元/g, "点") + " 份";
	}
...
function validateForm(){
	// 在键盘按下并释放及提交后验证提交表单
	redeemQuery = $("#redeemQuery").validate({
		rules: {
			tradeacco: {
				checkTradeacco: true
			},
 ...
 */
function redeemAll(){
	var allredeem = $(".available").val();
	if(allredeem == "" || allredeem == null || allredeem == undefined){
		allredeem = 0;
	}
	   			...
	   			// var  curdate = new Date().Format("yyyyMMdd");
	   			 var  curdate = $("#hidworkdate").val();			    			
	   				 if(allowredemptdate == curdate){
	   				       allredeem = allowredemptamt.replace(/(^\s*)|(\s*$)/g, "");
							  // allredeem =  delcommafy(allredeem);					  
	...
	if(allAmt.checked){
		document.getElementById("subAmt").value = allredeem;
	} else {
		...
		success : function(data) {
			if(data.errcode == "0000"){
				...
				for (var k = 0; k < fundbalanceList.length; k++) {
					...
					appendHtml+='</td>';
					appendHtml+='<td style="font-weight: bold;">基金份额：</td>';
					appendHtml+='<td class="white-bg form-inner">';
				...
				//可用份额
				$(".avaliableDis").html(data.availableBalance.toFixed(2));				
	...
	ctools.confirm({text:title},function(isConfirm){
		if(isConfirm){
			$.ajax({
				...
				async: false, 
				dataType : "json",
				data: $('#redeem').serialize(),// 你的formid
				url : BASE_PATH +'capitalService/server/submitRedeem.xhtml',
				success : function(data) {
					if(data.errCode == "0000"){
