...
function getAllSubFunds(){
	...
	$.ajax({
		...
		success: function(data) {
			...
			$('#fundid').select2();//{allowClear: false,minimumResultsForSearch:Infinity}
		}
	});
}
...
function validateForm(){
	// 在键盘按下并释放及提交后验证提交表单
	purchaseQuery = $("#purchaseQuery").validate({
		rules: {
			tradeacco: {
				checkTradeacco: true
			},
...
jQuery.validator.addMethod("checkSubAmtSize",function(value,element){
	...
	if(subAmt != ""  && queryflag == "Y"){
		$.ajax({
			...
			success : function(data) {
				var minBidAmt = data.minBidAmt;
				if((Number(subAmt) < Number(minBidAmt))){
					success= false;
					customMsg = "申购金额不能小于最低限额："+minBidAmt+"！";
				}else{
					success= true;
				}
			},
	...
	ctools.confirm({text:title},function(isConfirm){
		if(isConfirm){
			$.ajax({
				...
				async: false, 
				dataType : "json",
				data: $('#purchase').serialize(),// 你的formid
				url : BASE_PATH +'capitalService/server/submitPurchase.xhtml',
				success : function(data) {
					if(data.errCode == "0000"){
