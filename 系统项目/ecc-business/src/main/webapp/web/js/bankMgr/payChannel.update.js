...
$(function() {
	...
	var bankNO=$('#bankNO').attr('val');
	var bankName=$('#bankNO').attr('text');
	WASP_WIDGET.initializeSelectVal('bankNO',bankNO,bankName);
	WASP_WIDGET.initializeSelectVal('payMode');
	WASP_WIDGET.initializeSelectVal('status');
	WASP_WIDGET.initializeSelectVal('mipFlag');
	WASP_WIDGET.initializeSelectVal('recommend');
	initValidateForm();
})
...
function doSubmit(method){
	...
	var actionUrl= PRIMARY_PATH + "/update.xhtml";
	$.ajax({
		type: 'POST',
		url: actionUrl,
		dataType:'json',
