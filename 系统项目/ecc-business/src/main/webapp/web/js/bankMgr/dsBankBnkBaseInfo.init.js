...
function initGrid() {
	dsBankBnkbaseInfoList.jqGrid({
				url : PRIMARY_PATH + '/dsBankBnkBaseInfo.xhtml',
				caption : '直销基本信息列表<button type="button" class="btn btn-default btn-sm btn-small btn-fullscreen" title="" tabindex="-1"><i class="fa fa-arrows-alt icon-fullscreen"></i></button>',
				datatype : "json",
				mtype:"GET",
				...
				pager : "#dsBankBnkbaseInfoPage",
				viewrecords : true,
				hidegrid : false,
				gridComplete : function() {
					var ids = dsBankBnkbaseInfoList.jqGrid('getDataIDs');
					for (var i = 0; i < ids.length; i++) {
						var id = ids[i];
						var rowData = dsBankBnkbaseInfoList.jqGrid('getRowData', id);
						...
						dsBankBnkbaseInfoList.jqGrid('setRowData', id, {
							option : be + se
						});
					}
				},
	...
	dsBankBnkbaseInfoList.navGrid('#dsBankBnkbaseInfoPage', {
		edit : false,
		add : false,
		del : false,
		search : false,
	...
	dsBankBnkbaseInfoList.jqGrid('setFrozenColumns');
	jqGridResize(dsBankBnkbaseInfoList);
};
// 对按钮绑定全屏事件
...
function queryByCondtion(flag){
	var postData = dsBankBnkbaseInfoList.jqGrid("getGridParam", "postData");
    $.extend(postData,queryPostData());
    if (flag) {
    	dsBankBnkbaseInfoList.jqGrid("setGridParam",{"datatype": "json"}).trigger("reloadGrid",[{page:1}]);//重新载入Grid表格
    } else {
    	dsBankBnkbaseInfoList.jqGrid("setGridParam",{"datatype": "json"}).trigger("reloadGrid");//重新载入Grid表格
    }
}

function queryPostData(){
	var postData={};
	var bnkNo = $("#bankno").val();
	var bankName = $("#banknm").val();
	postData.sp={};
	postData.sp.bnkNo = bnkNo;
	postData.sp.bankName = bankName;	
	return postData;
}
