...
function search4Input(){
	...
	$.each(allSet,function(index,item){//从所有的集合中查找出用户输入的关键字的基金代码放入查找集合
		console.info(item);
...
function save(){
	SubmitAndPreventSecond("privilegeForm",function(){
		roleId = $("#roleId").val();
		var selValue = "";
		var data=[];
		...
		if($list.length>0){
				...
				$("#resId").val(selValue);
				$("#privilegeForm").attr("action","updatePrivilege.do");
		}else{
			$("#resId").val(selValue);
			$("#privilegeForm").attr("action","updatePrivilege.do");
		}
...
function selectBox(f){  
	  ...
	  url = PATH_PREFIX +"queryResourceForSel.do";
			...
			success: function(data){
				...
				if(data && data.length>0){
					...
					for(var i=0;i<data.length;i++){
						...
						allSet[i]=_item;//将所有的基金代码保存到一个集合，用于本地搜索
