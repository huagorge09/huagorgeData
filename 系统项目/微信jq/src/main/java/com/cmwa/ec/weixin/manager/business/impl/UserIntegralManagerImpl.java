...
import com.cmwa.ec.weixin.model.integral.IntegralDetailVO;
...
public class UserIntegralManagerImpl implements UserIntegralManager {
    ...
    public JSONObject getUserIntegralInfo(HttpServletRequest request, String cmfUserId) {
        ...
        try {
            ...
            if (null != userBaseInfo && userResult.isSuccess()) {
                ...
                result = new JSONObject().accumulate(WXConstants.ERR_CODE, WXConstants.COMMON_SUCCESS)
                    ...
                    .accumulate("invitationCode", integralInfo.getInvitationCode())
                    .accumulate("alreadyPurchase", integralInfo.getAlreadyPurchase());
            } else {
                LOGGER.error("getUserIntegralInfo_获取用户积分信息失败_errCode:{}, errMsg:{}",
                    userResult.getErrorCode(), userResult.getErrorMsg());
            }
        } catch (Exception omg) {
            LOGGER.error("getUserIntegralInfo_获取用户积分信息异常_", omg);
        }
        return result;
    }
    ...
    public List<IntegralDetailVO> listUserIntegralDetail(String cmfUserId) {
        ...
        try {
            ...
            if (userResult.isSuccess() && WXConstants.COMMON_SUCCESS.equals(queryResult.getReturnCode())) {
...
/*            List<ParameterDto> listParaCc = (List<ParameterDto>) queryParamListCc.getData();
        ...
        if(msgServicedto!=null){
            sendSuccess = WXConstants.COMMON_SUCCESS.equals(msgServicedto.getResultCode());
        }
        return sendSuccess;
    }
