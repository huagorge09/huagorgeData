...
public class WealthFocusEvent extends BaseEvent {

	private static Logger logger = Logger.getLogger(WealthFocusEvent.class.getName());
	
	public WealthFocusEvent(){
	...
	public String getRes(BaseMsgDto reqDto, ParameterDto parDto) {
		...
		//记录为空
		answer = "<xml><ToUserName>{openid}</ToUserName><FromUserName>{weiXinUserName}</FromUserName><CreateTime>{timestamp}</CreateTime><MsgType>text</MsgType><Content>您好，系统暂未添加数据，我们会尽快更新！敬请期待！</Content></xml>";
