...
var legion = {
	...
	regValidater : {
		...
		isIdCardNumber4ZSJJ : function(val) {
			...
			var month = Number(val.substr(10, 2));
			var day = Number(val.substr(12, 2));
			
			sBirthday = val.substr(6, 4) + "-" + (month<10?("0"+month):month) + "-"
					+ (day<10?("0"+day):day);
			
			var d = new Date(sBirthday.replace(/-/g, "/"));
			//var d = new Date("1989/04/16");
			var m = d.getMonth() + 1;
			var dd = d.getDate();
			var tempTime = d.getFullYear() + "-" + (m<10?("0"+m):m) + "-" + (dd<10?("0"+dd):dd);
			var tempTime1 = d.getFullYear() + "-" + (m<10?("0"+m):m) + "-" + ((dd<10?("0"+dd):dd)+1);
			
			if (sBirthday != tempTime && sBirthday != tempTime1 )
				//return "您输入的身份证出生日期非法";
				return "请您输入正确的身份证号码";
			for ( var i = 17; i >= 0; i--)
