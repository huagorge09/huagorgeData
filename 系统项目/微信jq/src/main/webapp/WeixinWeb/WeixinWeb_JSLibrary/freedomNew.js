 ...
 * 4.不能包含特殊符号
 */
Validater.isTradePasswordNew = function (_val) {
	return (/^[\da-zA-Z]{6,16}$/.test(_val)//长度6~16位
			&& (Validater.hasNumber(_val) && Validater.hasLetter(_val))
			&& !Validater.hasBanJiaoSpacialChar(_val) && !Validater.hasQuanJiaoSpacialChar(_val));//不包括半角和全角特殊符号
}
/**
 ...
 */
Validater.isTradePassword01 = function (_val) {
	return (/^[\da-zA-Z\~\!\@\#\$\%\^\&\*\(\)\_\+\{\}\|\:\"\<\>\?\-\=\[\]\\\;\'\,\.\/]{6,16}$/.test(_val)//长度6~16位
...
WebpageLoader.load = function (_reqUrl, _loadObj, _options) {
	...
	var defaultOptions = {isOpenWaitCursor:false, isIFrame:false, isOpenUnload:false, isAppend:false, reqData:null, reqType:"post", isAsync:true, isCache:false, backFn:function () {}, backFnParams:null, manualBackFn:function () {}, manualBackFnParams:null};
...
IFrame.open = function (_contentUrl, _options) {
	...
	if (Validater.isUndefined($("#maodIFrameWindow"))) {
		...
		style += "<style>";
			...
			style += ".maodIFrameShade {z-index:999999; width:100%; height:100%; display:block; position:fixed; background-color:#000; top:0px; left:0px; filter:alpha(opacity=30); opacity:0.3;}";
			style += (".maodIFrameMain {z-index:999999; width:" + width + "px; height:" + height + "px; display:block; position:" + IFrame_position + "; background-color:#FFF; top:" + marginTopPx + "px; left:" + marginLeftPx + "px; box-shadow:4px 4px 10px #000;}");
			style += (".maodIFrameHead {z-index:999999; width:" + (width - 10) + "px; height:20px; line-height:20px; padding:5px;}");
			style += (".maodIFrameHead a {width:15px; height:15px; display:inline-block; position:absolute; top:10px; right:10px; background:url(\"" + imgAbsolutePath.close + "\") no-repeat scroll -18px 0px transparent;}");
		...
		$("#maodIFrameWindow>div:last").append("<div id=\"maodIFrameHead\" class=\"maodIFrameHead\" ondblclick=\"javascript:IFrame.destroy();\"><b>" + finalOptions.title + "</b><a href=\"javascript:Freedom.nothing();\" onclick=\"javascript:IFrame.close();\"></a></div>");
...
Freedom.hint4clear = function (_tarJQueryObj, _hintText) {
	...
	if (_tarJQueryObj.val().trim() == "" || _tarJQueryObj.val().trim() == oldHintText
		...
		_tarJQueryObj.css("color", "#666666");
	}
