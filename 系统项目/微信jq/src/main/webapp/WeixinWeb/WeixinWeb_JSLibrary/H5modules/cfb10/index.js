...
initPage();
localStorage.setItem("type", "cfb10")

function initPage() {
    localStorage.setItem("target", "cfb10/guide");
            ...
            if (data.success) {
                ...
                if (data.response.returnCode == "Y") {  //已绑定
                    if(location.href.indexOf("index")<0&&location.href.indexOf("bussinessCard")<0){
                        location.href = "/WeixinService/H5modules/H5Content/cfb10/index.html?eventId="+eventId+"&pageSource="+pageSource;
					}
                }
            }
        },
	...
	$.ajax({
			...
			success : function(data) {
				if(data.success){
					...
					$("#saveBtn").addClass("disabled").attr("onclick","");
				    operatingRecord(pageSource,"event_cfb10_subscribeId",pageId,"","","","")
				}else{
...
function elemBind(){
	...
	$(".once1").click(function(){
		operatingRecord(pageSource,"event_cfb10_buyId","","","","","")
 ...
 */
function bussiness(){
     operatingRecord(pageSource,"event_cfb10_interestId",pageId,"","","","")
 ...
 */
function myAdviser(){
	location.href="/WeixinService/H5modules/H5Content/cfb10/bussinessCard.html?pageSource="+pageSource+"&eventId=event_cfb10_adviserId"
}
function seeProduct(){
	location.href="/WeixinService/H5modules/H5Content/cfb10/index.html?pageSource="+pageSource+"&eventId=event_cfb10_h5Id"
}
