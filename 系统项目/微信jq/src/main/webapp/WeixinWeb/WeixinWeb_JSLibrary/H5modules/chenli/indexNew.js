...
initPage();
localStorage.setItem("type", "chenli")

function initPage() {
    localStorage.setItem("target", "chenli/guide");
            ...
            if (data.success) {
                ...
                if (data.resp.returnCode == "Y") {  //已绑定
                    if(location.href.indexOf("index")<0&&location.href.indexOf("bussinessCard")<0){
                        location.href = "/WeixinService/H5modules/H5Content/chenli/index.html?eventId="+eventId+"&pageSource="+pageSource;
					}
                }
            }
        },
	...
	$.ajax({
			...
			success : function(data) {
				if(data.success){
					...
					$("#saveBtn").addClass("disabled").attr("onclick","");
				    operatingRecord(pageSource,"event_chenli_subscribeId",pageId,"","","","")
				}else{
...
function elemBind(){
	...
	$(".once1").click(function(){
		operatingRecord(pageSource,"event_chenli_buyId","","","","","")
 ...
 */
function bussiness(){
     operatingRecord(pageSource,"event_chenli_interestId",pageId,"","","","")
 ...
 */
function myAdviser(){
	location.href="/WeixinService/H5modules/H5Content/chenli/bussinessCard.html?pageSource="+pageSource+"&eventId=event_chenli_adviserId"
}
function seeProduct(){
	location.href="/WeixinService/H5modules/H5Content/chenli/index.html?pageSource="+pageSource+"&eventId=event_chenli_h5Id"
}
    ...
    $.ajax({
        ...
        success : function(data) {
            if(data.returnCode=="0000"){
            	...
            	var benefitSinceCreated=data.fundInfo.benefitSinceCreated?data.fundInfo.benefitSinceCreated:"--"
                $("#startBuy").text(numDiv(money,10000));
                $("#benefitSinceCreated").text(benefitSinceCreated);
            }
        },
        error : function() {
            errorRemark("网络繁忙，请稍后再试。");
