...
initPage();
localStorage.setItem("type", "cfb126")

function initPage() {
    localStorage.setItem("target", "cfb126/guide");
            ...
            if (data.success) {
                ...
                if (data.response.returnCode == "Y") {  //已绑定
                    if(location.href.indexOf("index")<0&&location.href.indexOf("bussinessCard")<0){
                        location.href = "/WeixinService/H5modules/H5Content/cfb126/index.html?eventId="+eventId+"&pageSource="+pageSource;
					}
                }
            }
        },
	...
	$.ajax({
			...
			success : function(data) {
				if(data.success){
					...
					$("#saveBtn").addClass("disabled").attr("onclick","");
				    operatingRecord(pageSource,"event_cfb126_subscribeId",pageId,"","","","")
				}else{
 ...
 */
function bussiness(){
     operatingRecord(pageSource,"event_cfb126_interestId",pageId,"","","","")
 ...
 */
function myAdviser(){
	location.href="/WeixinService/H5modules/H5Content/cfb126/bussinessCard.html?pageSource="+pageSource+"&eventId=event_cfb126_adviserId"
}
function seeProduct(){
	location.href="/WeixinService/H5modules/H5Content/cfb126/index.html?pageSource="+pageSource+"&eventId=event_cfb126_h5Id"
}
