...
initPage();
localStorage.setItem("type", "annualBonus")

function initPage() {
    localStorage.setItem("target", "annualBonus/guide");
            ...
            if (data.success) {
                ...
                if (data.resp.returnCode == "Y") {  //已绑定
                    if(location.href.indexOf("index")<0&&location.href.indexOf("bussinessCard")<0){
                        location.href = "/WeixinService/H5modules/H5Content/annualBonus/index.html?eventId="+eventId+"&pageSource="+pageSource;
					}
                }
            }
        },
	...
	$.ajax({
			...
			success : function(data) {
				if(data.success){
					...
					$("#saveBtn").addClass("disabled").attr("onclick","");
				    operatingRecord(pageSource,"event_annualBonus_subscribeId",pageId,"","","","")
				}else{
...
function elemBind(){
    try{
		...
		$(".makeBuy1").click(function(){
			operatingRecord(pageSource,"event_annual7Days_buyId","","","","","")
            redirectUrl("/WeixinService/business/query/fundInfoNew.shtml?fundId=17D987&period=1")
		});
		$(".makeBuy2").click(function(){
			operatingRecord(pageSource,"event_annualchangli_buyId","","","","","")
			redirectUrl("/WeixinService/business/query/fundInfoNew.shtml?fundId=17D969&period=1")
		});
		$(".makeBuy3").click(function(){
			bussiness()
		});
    }catch(e){
 ...
 */
function bussiness(){
     operatingRecord(pageSource,"event_annualBonus_interestId",pageId,"","","","")
 ...
 */
function myAdviser(){
	location.href="/WeixinService/H5modules/H5Content/annualBonus/bussinessCard.html?pageSource="+pageSource+"&eventId=event_annualBonus_adviserId"
}
function seeProduct(){
	location.href="/WeixinService/H5modules/H5Content/annualBonus/index.html?pageSource="+pageSource+"&eventId=event_annualBonus_h5Id"
}
...
function previewImageFun() {
    var obj = {
		urls : ['/WeixinWeb/WeixinWeb_Images/H5modules/annualBonus/img4.png'],
		current : '/WeixinWeb/WeixinWeb_Images/H5modules/annualBonus/img4.png'
    };
