...
initPage();
localStorage.setItem("type", "cfb203")

function initPage() {
    localStorage.setItem("target", "cfb203/guide");
            ...
            if (data.success) {
                ...
                if (data.response.returnCode == "Y") {  //已绑定
                    if(location.href.indexOf("index")<0&&location.href.indexOf("bussinessCard")<0){
                        location.href = "/WeixinService/H5modules/H5Content/cfb203/index.html?eventId="+eventId+"&pageSource="+pageSource;
					}
                }
            }
        },
	...
	$.ajax({
			...
			success : function(data) {
				if(data.success){
					...
					$("#saveBtn").addClass("disabled").attr("onclick","");
				    operatingRecord(pageSource,"event_cfb203_subscribeId",pageId,"","","","")
				}else{
...
function elemBind(){
	...
	$(".once1").click(function(){
		operatingRecord(pageSource,"event_cfb203_buyId","","","","","")
 ...
 */
function bussiness(){
     operatingRecord(pageSource,"event_cfb203_interestId",pageId,"","","","")
 ...
 */
function myAdviser(){
	location.href="/WeixinService/H5modules/H5Content/cfb203/bussinessCard.html?pageSource="+pageSource+"&eventId=event_cfb203_adviserId"
}
function seeProduct(){
	location.href="/WeixinService/H5modules/H5Content/cfb203/index.html?pageSource="+pageSource+"&eventId=event_cfb203_h5Id"
}
