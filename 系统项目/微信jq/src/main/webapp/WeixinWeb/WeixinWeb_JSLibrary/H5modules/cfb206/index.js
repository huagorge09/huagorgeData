...
initPage();
localStorage.setItem("type", "cfbQun")

function initPage() {
    localStorage.setItem("target", "cfbQun/guide");
            ...
            if (data.success) {
                ...
                if (data.response.returnCode == "Y") {  //已绑定
                    if(location.href.indexOf("index")<0&&location.href.indexOf("bussinessCard")<0&&location.href.indexOf("detail1")<0&&location.href.indexOf("detail2")<0){
                        location.href = "/WeixinService/H5modules/H5Content/cfbQun/index.html?eventId="+eventId+"&pageSource="+pageSource;
					}
                }
            }
        },
...
function userDateSumbit(userName,mobilePhone){
	var type="cfbQun";
	if(location.href.indexOf("detail1")>-1){
		type="cfb206"
	}else if(location.href.indexOf("detail2")>-1){
		type="qunOne"
	}
	$.ajax({
			async : false,
			...
			success : function(data) {
				if(data.success){
					...
					var type="event_cfbQun_subscribeId";
					if(location.href.indexOf("detail1")>-1){
						type="event_cfb206_subscribeId" 
					}else if(location.href.indexOf("detail2")>-1){
						type="event_qunOne_subscribeId" 
					}
...
function elemBind(){
	...
	$(".once1").click(function(){
		operatingRecord(pageSource,"event_cfb206_buyId",pageId,"","","","")
		redirectUrl("/WeixinService/business/query/fundInfo.shtml?fundId="+fundId+"&period="+period);
    });
    //群星一号
    $(".once2").click(function(){
		operatingRecord(pageSource,"event_qunOne_buyId",pageId,"","","","")
 ...
 */
function bussiness(){
	var type="event_cfbQun_interestId";
	if(location.href.indexOf("detail1")>-1){
		type="event_cfb206_interestId" 
	}else if(location.href.indexOf("detail2")>-1){
		type="event_qunOne_interestId" 
	}
     operatingRecord(pageSource,type,pageId,"","","","")
 ...
 */
function myAdviser(){
	location.href="/WeixinService/H5modules/H5Content/cfbQun/bussinessCard.html?pageSource="+pageSource+"&eventId=event_cfbQun_adviserId"
}
function seeProduct(){
	location.href="/WeixinService/H5modules/H5Content/cfbQun/index.html?pageSource="+pageSource+"&eventId=event_cfbQun_h5Id"
}

function seeProductDetailCfb(){
	location.href="/WeixinService/H5modules/H5Content/cfbQun/detail1.html?pageSource="+pageSource+"&eventId=event_cfbDetail_h5Id"
}
function seeProductDetailQunxing(){
	location.href="/WeixinService/H5modules/H5Content/cfbQun/detail2.html?pageSource="+pageSource+"&eventId=event_qunDetail_h5Id"
}
