...
initPage();
localStorage.setItem("type", "chenrui")

function initPage() {
    localStorage.setItem("target", "chenrui/guide");
            ...
            if (data.success) {
                ...
                if (data.resp.returnCode == "Y") {  //已绑定
                    if(location.href.indexOf("index")<0&&location.href.indexOf("bussinessCard")<0){
                        location.href = "/WeixinService/H5modules/H5Content/chenrui/index.html?eventId="+eventId+"&pageSource="+pageSource;
					}
                }
            }
        },
	...
	$.ajax({
			...
			success : function(data) {
				if(data.success){
					...
					$("#saveBtn").addClass("disabled").attr("onclick","");
				    operatingRecord(pageSource,"event_chenrui_subscribeId",pageId,"","","","")
				}else{
 ...
 */
function elemBind(){
	var fundId=queryParamList1("SYSTEM","H5ProUrlFundId2","")[0].pmco;
    var period=queryParamList1("SYSTEM","H5ProUrlPeriod2","")[0].pmco;
    queryProductInfo(fundId,period)
	$(".once1").click(function(){
		operatingRecord(pageSource,"event_chenrui_buyId","","","","","")
 ...
 */
function bussiness(){
     operatingRecord(pageSource,"event_chenrui_interestId",pageId,"","","","")
 ...
 */
function myAdviser(){
	location.href="/WeixinService/H5modules/H5Content/chenrui/bussinessCard.html?pageSource="+pageSource+"&eventId=event_chenrui_adviserId"
}
function seeProduct(){
	location.href="/WeixinService/H5modules/H5Content/chenrui/index.html?pageSource="+pageSource+"&eventId=event_chenrui_h5Id"
}
