...
$(function () {
    ...
    localStorage.setItem("type","heli");
    window.onbeforeunload=function(){
         operatingRecord(pageSource, "event_hlNo5_close", pageId, "", "", "", "")
    }
});
/* 验证输入框的有效性  */
    ...
    $.ajax({
        ...
        success: function (data) {
            if (data.success) {
                ...
                $("#saveBtn").addClass("disabled").attr("onclick", "");
                operatingRecord(pageSource, "event_hlNo5_subscribeId", pageId, "", "", "", "");
                setTimeout(function () {
 ...
 */
function elemBind() {
    var params = "fundId=17D974&period=1";
    $(".oncePro").click(function () {
        operatingRecord(pageSource, "event_hlNo5_proDetailBtnH5Id",pageId, "", "", "", "");
        redirectUrl("/WeixinService/business/query/fundInfoNew.shtml?" + params);
    });

    $('.seeKan').click(function () {
        operatingRecord(pageSource, "event_hlNo5_proDetailAreaH5Id", pageId, "", "", "", "");
        redirectUrl("/WeixinService/business/query/fundInfoNew.shtml?" + params);
    });
    //鼠标键盘弹起
 ...
 */
function bussiness() {
    operatingRecord(pageSource, "event_hlNo5_interestId", pageId, "", "", "", "");  //远航5号
 ...
 */
function myAdviser() {
    location.href = "/WeixinService/H5modules/H5Content/heli/bussinessCard.html?pageSource=" + pageSource + "&eventId=event_hlNo5_adviserId"
}
function seeProduct() {
    location.href = "/WeixinService/H5modules/H5Content/heli/index.html?pageSource=" + pageSource + "&eventId=event_hlNo5_h5Id"
}
