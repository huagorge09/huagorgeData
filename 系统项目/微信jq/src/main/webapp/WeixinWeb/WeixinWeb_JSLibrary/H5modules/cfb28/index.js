...
initPage();
localStorage.setItem("type", "cfb28")

function initPage() {
    localStorage.setItem("target", "cfb28/guide");
    if (!userid) { //判断用户有没有静默授权
        ...
        if (pageSource !== "") {
            if(pageSource=="event_cfb28_wxImgTextBMW"){
                operatingRecord(pageSource, "event_cfb28_wxImgTextBMW", pageId, "", "", "", "")
            }else{
            ...
            if (data.success) {
                ...
                if (data.response.returnCode == "Y") { //已绑定
                    if (location.href.indexOf("index") < 0 && location.href.indexOf("bussinessCard") < 0) {
                        location.href = "/WeixinService/H5modules/H5Content/cfb28/index.html?eventId=" + eventId + "&pageSource=" + pageSource;
                    }
                }
            }
        },
    ...
    $.ajax({
        ...
        success: function (data) {
            if (data.success) {
                ...
                $("#saveBtn").addClass("disabled").attr("onclick", "");
                operatingRecord(pageSource, "event_cfb28_subscribeId", pageId, "", "", "", "")
            } else {
 ...
 */
function bussiness() {
    operatingRecord(pageSource, "event_cfb28_interestId", pageId, "", "", "", "")
 ...
 */
function myAdviser() {
    location.href = "/WeixinService/H5modules/H5Content/cfb28/bussinessCard.html?pageSource=" + pageSource + "&eventId=event_cfb28_adviserId"
}

function seeProduct() {
    location.href = "/WeixinService/H5modules/H5Content/cfb28/index.html?pageSource=" + pageSource + "&eventId=event_cfb28_h5Id"
}
