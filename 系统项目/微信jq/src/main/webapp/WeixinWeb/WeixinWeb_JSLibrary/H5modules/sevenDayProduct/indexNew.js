...
$(function(){
    ...
    localStorage.setItem("type","7DaysPro");
    queryThereNetValue()
    queryBusiness()
});
/* 验证输入框的有效性  */
	...
	$.ajax({
			...
			success : function(data) {
				if(data.success){
					...
					$("#saveBtn").addClass("disabled").attr("onclick","");
				    operatingRecord(pageSource,"event_7DaysPro_subscribeId",pageId,"","","","")
				}else{
 ...
 */
function elemBind(){
	var params="fundId=17D987&period=1"
	$(".once").click(function(){
		operatingRecord(pageSource,"event_7DaysPro_buyId","","","","","")
 ...
 */
function bussiness(){	
     operatingRecord(pageSource,"event_7DaysPro_interestId",pageId,"","","","")
 ...
 */
function myAdviser(){
	location.href="/WeixinService/H5modules/H5Content/sevenDayProduct/bussinessCard.html?pageSource="+pageSource+"&eventId=event_7DaysPro_adviserId"
}
function seeProduct(){
	location.href="/WeixinService/H5modules/H5Content/sevenDayProduct/index.html?pageSource="+pageSource+"&eventId=event_7DaysPro_h5Id"
}
