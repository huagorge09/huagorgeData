...
initPage();
localStorage.setItem("type", "cfb209")

function initPage() {
    localStorage.setItem("target", "cfb209/guide");
            ...
            if (data.success) {
                ...
                if (data.response.returnCode == "Y") {  //已绑定
                    if(location.href.indexOf("index")<0&&location.href.indexOf("bussinessCard")<0&&location.href.indexOf("detail1")<0&&location.href.indexOf("detail2")<0){
                        location.href = "/WeixinService/H5modules/H5Content/cfb209/index.html?eventId="+eventId+"&pageSource="+pageSource;
					}
                }
            }
        },
...
var activity ='';
function seeProductyou(data) {
    var activity209 = "cfb209"
    var activity126 = "cfb126"
    if(data == 1){
	...
	$.ajax({
			...
			success : function(data) {
				if(data.success){
				    ...
				    operatingRecord(pageSource,"event_cfb209_subscribeId",pageId,"","","","")
				}else{
...
function elemBind(){
	...
	$(".once1").click(function(){
		operatingRecord(pageSource,"event_cfb209_buyId",pageId,"","","","")
		redirectUrl("/WeixinService/business/query/fundInfo.shtml?fundId="+fundId+"&period="+period);
    });
    $(".once2").click(function(){
		operatingRecord(pageSource,"event_cfb126_buyId",pageId,"","","","")
		redirectUrl("/WeixinService/business/query/fundInfo.shtml?fundId="+fundId2+"&period="+period2);
    });
 ...
 */
function bussiness(){
	var type="event_cfb209_interestId";
 ...
 */
function myAdviser(){
	location.href="/WeixinService/H5modules/H5Content/cfb209/bussinessCard.html?pageSource="+pageSource+"&eventId=event_cfb209_adviserId"
}
function seeProduct(){
	location.href="/WeixinService/H5modules/H5Content/cfb209/index.html?pageSource="+pageSource+"&eventId=event_cfb209_h5Id"
}

//function seeProductDetailCfb(){
//	location.href="/WeixinService/H5modules/H5Content/cfb207/detail1.html?pageSource="+pageSource+"&eventId=event_cfbDetail_h5Id"
//}
//function seeProductDetailQunxing(){
//	location.href="/WeixinService/H5modules/H5Content/cfb207/detail2.html?pageSource="+pageSource+"&eventId=event_qunDetail_h5Id"
//}
    ...
    $.ajax({
        ...
        success : function(data) {
            if(data.returnCode=="0000"){
                ...
                $("#subdeadLineA").text(dataNumber(data.fundInfo.subdeadLine))
            }
        },
        error : function() {
            errorRemark("网络繁忙，请稍后再试。");
...
// 转换时间格式
function dataNumber(date){
    var data = new Date(date)
    var year = data.getFullYear();
    var month = data.getMonth()+1;    //js从0开始取 
