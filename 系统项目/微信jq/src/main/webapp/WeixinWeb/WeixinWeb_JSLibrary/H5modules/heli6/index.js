...
$(function () {
    ...
    localStorage.setItem("type","heli6");
    window.onbeforeunload=function(){
         operatingRecord(pageSource, "event_hlNo6_close", pageId, "", "", "", "")
    }
});
/* 验证输入框的有效性  */
    ...
    $.ajax({
        ...
        success: function (data) {
            if (data.success) {
                ...
                $("#saveBtn").addClass("disabled").attr("onclick", "");
                operatingRecord(pageSource, "event_hlNo6_subscribeId", pageId, "", "", "", "");
                setTimeout(function () {
 ...
 */
function elemBind() {
    var params = "fundId=17D985&period=1";
    $(".oncePro").click(function () {
        operatingRecord(pageSource, "event_hlNo6_proDetailBtnH5Id",pageId, "", "", "", "");
        redirectUrl("/WeixinService/business/query/fundInfo.shtml?" + params);
    });

    $('.seeKan').click(function () {
        operatingRecord(pageSource, "event_hlNo6_proDetailAreaH5Id", pageId, "", "", "", "");
        redirectUrl("/WeixinService/business/query/fundInfo.shtml?" + params);
    });
    //鼠标键盘弹起
 ...
 */
function bussiness() {
    operatingRecord(pageSource, "event_hlNo6_interestId", pageId, "", "", "", "");  //远航5号
...
function spread(e) {
    if($(".remark").is(":visible")){
        $(".remark").slideUp();
        $(e).html("展开").removeClass("slide");

    }else{
        $(".remark").slideDown()
        $(e).html("收起").addClass("slide");
    }
}
/**
 *我的顾问
 */
function myAdviser() {
    location.href = "/WeixinService/H5modules/H5Content/heli6/bussinessCard.html?pageSource=" + pageSource + "&eventId=event_hlNo5_adviserId"
}
function seeProduct() {
    location.href = "/WeixinService/H5modules/H5Content/heli6/index.html?pageSource=" + pageSource + "&eventId=event_hlNo5_h5Id"
}
