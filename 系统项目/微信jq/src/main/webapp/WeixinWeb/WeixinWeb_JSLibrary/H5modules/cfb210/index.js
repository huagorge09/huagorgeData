...
initPage();
localStorage.setItem("type", "cfb210")

function initPage() {
    localStorage.setItem("target", "cfb210/guide");
            ...
            if (data.success) {
                ...
                if (data.response.returnCode == "Y") {  //已绑定
                    if(location.href.indexOf("index")<0&&location.href.indexOf("bussinessCard")<0&&location.href.indexOf("detail1")<0&&location.href.indexOf("detail2")<0){
                        location.href = "/WeixinService/H5modules/H5Content/cfb210/index.html?eventId="+eventId+"&pageSource="+pageSource;
					}
                }
            }
        },
	...
	$.ajax({
			...
			success : function(data) {
				if(data.success){
				    ...
				    operatingRecord(pageSource,"event_cfb210_subscribeId",pageId,"","","","")
				}else{
...
function elemBind(){
	...
	$(".once1").click(function(){
		operatingRecord(pageSource,"event_cfb210_buyId",pageId,"","","","")
 ...
 */
function bussiness(){
	var type="event_cfb210_interestId";
 ...
 */
function myAdviser(){
	location.href="/WeixinService/H5modules/H5Content/cfb210/bussinessCard.html?pageSource="+pageSource+"&eventId=event_cfb210_adviserId"
}
function seeProduct(){
	location.href="/WeixinService/H5modules/H5Content/cfb210/index.html?pageSource="+pageSource+"&eventId=event_cfb210_h5Id"
}
