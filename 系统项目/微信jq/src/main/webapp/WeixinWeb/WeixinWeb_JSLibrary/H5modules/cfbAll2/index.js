...
initPage();
localStorage.setItem("type", "cfbAll2")

function initPage() {
    localStorage.setItem("target", "cfbAll2/guide");
            ...
            if (data.success) {
                ...
                if (data.resp.returnCode == "Y") {  //已绑定
                    if(location.href.indexOf("index")<0&&location.href.indexOf("bussinessCard")<0){
                        location.href = "/WeixinService/H5modules/H5Content/cfbAll2/index.html?eventId="+eventId+"&pageSource="+pageSource;
					}
                }
            }
        },
...
function userDateSumbit(userName,mobilePhone){
    var typeId=$("input[name=typeId]").val()?$("input[name=typeId]").val():"cfbAll2";
    var eventType="event_"+typeId+"_subscribeId"
	$.ajax({
			async : false,
			...
			success : function(data) {
				if(data.success){
					...
					$("#saveBtn").addClass("disabled").attr("onclick","");
				    operatingRecord(pageSource,eventType,pageId,"","","","")
				}else{
...
function elemBind(){
	...
	$(".once1").click(function(){
		operatingRecord(pageSource,"event_cfb28_buyId","","","","","")
 ...
 * 预约
 */
function bussiness(type){
	 var type=type||cfbAll2   
	 var eventType="event_"+type+"_interestId"
     operatingRecord(pageSource,eventType,pageId,"","","","")
	 $(".reserves").removeClass("hide");
	 $("input[name=typeId]").val(type)
 ...
 */
function myAdviser(){
	location.href="/WeixinService/H5modules/H5Content/cfbAll2/bussinessCard.html?pageSource="+pageSource+"&eventId=event_cfbAll2_adviserId"
}

function seeProduct(){
	location.href="/WeixinService/H5modules/H5Content/cfbAll2/index.html?pageSource="+pageSource+"&eventId=event_cfbAll2_h5Id"
}
