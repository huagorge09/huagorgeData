...
function queryAddressInfo() {
	...
	$("#vocName").val(vocName);
	$("#vocNameText").html(!vocName ? "请选择":vocName);
...
function saveAddress(){
	...
	$.ajax({
        async: !1,
        url: apiHost + "/api/updateCmfUserBaseInfo",
        data: {
        	nation : nation,
        	province : province,
        	city : city,
	...
	"cmfUserId":localStorage.getItem("cmfUserId")
        ...
        success: function(n) {
        	if(n.success){
        		if(n != null && n.resp.returnCode=="0000"){
	        		redirectUrl("/WeixinService/H5modules/riskAssessment/riskLevel.html?viewType=prodcut&birthDate="+dateOfBirth
...
function cacelAddressUpdate(){
	window.location.href="/WeixinService/H5modules/H5Content/"+localStorage.getItem("target")+".html";
}
...
function loadNationInfo(){
	...
	$("#section_01 a.href-point").click(function(){
		...
		if(emLen > 0){
			window.location.href="/WeixinService/H5modules/riskAssessment/fundProvinceInfo.html" + param;
		}else{
...
function loadProvinceInfo() {
	...
	$("#section_02 a.href-point").click(function(){
		...
		var emLen = $(this).find("em").length;
		if(emLen > 0){
			window.location.href="/WeixinService/H5modules/riskAssessment/fundCityInfo.html" + param;
		}else{
			window.location.href="/WeixinService/H5modules/riskAssessment/fundModRiskHouseAddr.html"+param;
		}
	})
...
function goFundProfessionInfoView(){
	...
	redirectUrl('/WeixinService/H5modules/riskAssessment/fundProfessionInfo.html' + param)
};
...
function goFundNationInfoView(){
	...
	redirectUrl('/WeixinService/H5modules/riskAssessment/fundNationInfo.html' +param);
};
...
function goFundTaxResidentTypeView(){
	...
	redirectUrl('/WeixinService/H5modules/riskAssessment/fundTaxResidentType.html' +param);
}
...
function goFundTaxNationInfoView(nationType,targetId){
	...
	var param =getAllQueryString()+"&nationType="+nationType+"&targetId="+targetId+"&taxResidentData="+encodeURI(taxResidentData)
				+"&eventId="+eventId+"&pageSource="+pageSource;
	redirectUrl('/WeixinService/H5modules/riskAssessment/fundTaxNationInfo.html' +param);
};
...
function goFundTaxNotCodeCauseView(targetId){
	...
	+"&eventId="+eventId+"&pageSource="+pageSource;
	redirectUrl('/WeixinService/H5modules/riskAssessment/fundTaxNotCodeCause.html' +param);
}
...
function saveUserTaxInfoToAddrView(){
	...
	if("product" == sourceType){
		...
		redirectUrl('/WeixinService/H5modules/riskAssessment/updateTaxResidentInfo.html' + param);
	}
}
...
function queryUserTaxInfo(){
	...
	$.ajax({
        async: !1,
        url: apiHost + "/api/queryUserTaxInfo",
        data: {
		"cmfUserId":localStorage.getItem("cmfUserId")
        },
        dataType: "json",
        ...
        success: function(data) {
        	if(!!data){
        		taxResidentData = JSON.stringify(data.resp);
        	}
        }
    });
	return taxResidentData;
		...
		success : function(data) {
			if(data.success){
				if (data.resp.returnCode != null && data.resp.returnCode == "0000") {
					$("#userCustName").val(data.resp.custName);
				}
			}
