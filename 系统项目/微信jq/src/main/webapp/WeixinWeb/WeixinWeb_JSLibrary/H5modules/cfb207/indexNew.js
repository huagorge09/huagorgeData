...
initPage();
localStorage.setItem("type", "cfb207")

function initPage() {
    localStorage.setItem("target", "cfb207/guide");
            ...
            if (data.success) {
                ...
                if (data.response.returnCode == "Y") {  //已绑定
                    if(location.href.indexOf("index")<0&&location.href.indexOf("bussinessCard")<0&&location.href.indexOf("detail1")<0&&location.href.indexOf("detail2")<0){
                        location.href = "/WeixinService/H5modules/H5Content/cfb207/index.html?eventId="+eventId+"&pageSource="+pageSource;
					}
                }
            }
        },
	...
	$.ajax({
			...
			success : function(data) {
				if(data.success){
				    ...
				    operatingRecord(pageSource,"event_cfb207_subscribeId",pageId,"","","","")
				}else{
...
function elemBind(){
	...
	$(".once1").click(function(){
		operatingRecord(pageSource,"event_cfb207_buyId",pageId,"","","","")
 ...
 */
function bussiness(){
	var type="event_cfb207_interestId";
 ...
 */
function myAdviser(){
	location.href="/WeixinService/H5modules/H5Content/cfb207/bussinessCard.html?pageSource="+pageSource+"&eventId=event_cfb207_adviserId"
}
function seeProduct(){
	location.href="/WeixinService/H5modules/H5Content/cfb207/index.html?pageSource="+pageSource+"&eventId=event_cfb207_h5Id"
}
