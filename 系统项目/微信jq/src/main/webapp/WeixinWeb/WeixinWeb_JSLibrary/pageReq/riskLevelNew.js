...
$(document).ready(function(e) {
	...
	if(VIEW_TYPE_PRODUCT == viewType){
		...
		$("#section_01 a[class=riskevaluation-btn]").attr("href","/#/fundList");
		toRisk();
	}
...
function toRisk() {
	...
	if(!birthDate){
		redirectUrl('/WeixinService/business/user/updateBirthDateNew.shtml');
		return;
	}
	//记录日志
				...
				if(!flag){
					...
					if(openMgm=="1"){
						...
						if(mgmFundId){
							...
							$("#myTreasure").attr("href",'/#/fundInfo/'+mgmFundId+'/'+mgmPeriod)
						}else{
							//需要实名
							$('#isAuth').show();
							$('#myTreasure').hide();
...
//							}, 4000); 
					        ...
					        t_delay = setInterval(function() {
						        time--;
						        $('#count-down').text("将自动进入实名认证")
						        $("#time-second").text(time+"s")
						        if (time == 0) {
						          clearInterval(t_delay);
			         			  redirectUrl('/#/cardinfo');
						        }
...
function applyInvestor(){
	...
	$.ajax({
        ...
        success: function(n) {
            ...
            }else if(n.resultCode =="9999"){
            	errorRemark("当前正在申请中，请勿重复申请!")
            }else{
