...
/*返回开始答题页面*/
function toIndex() {
	goToURL("/WeixinService/activity/magpieFestivalActivity/leadStartNew.shtml");
}
$("section ul li").click(
		function() {
	...
	$.ajax({
		...
		success : function(data) {
			if(data.returnCode=="0000"){
				...
				index =11;
				activityStatisticalLog(index);/* 此处subPath为页面内行为 */
        		redirectUrl("/WeixinService/activity/magpieFestivalActivity/leadSuccessNew.shtml");
        	}else if(data.returnCode=="0030"){
        		/*奖品已领完*/
        		index =12;
        		activityStatisticalLog(index);/* 此处subPath为页面内行为 */
        		redirectUrl("/WeixinService/activity/magpieFestivalActivity/leadEndNew.shtml");
        	}else if(data.returnCode=="0020"){
        		/*已经领取过*/
        		index =13;
        		activityStatisticalLog(index);/* 此处subPath为页面内行为 */
				redirectUrl("/WeixinService/activity/magpieFestivalActivity/leadPastNew.shtml");
