	...
	$.ajax({
		...
		success: function (n) {
			if (n != null && n.resultCode == "0000") {
				// 继续测评流程
				// redirectUrl("/WeixinService/business/query/fundListNew.shtml?riskContinue=Y");

				redirectUrl("/WeixinService/business/user/riskLevelNew.shtml?viewType=prodcut&birthDate=" + dateOfBirth
...
function loadNationInfo() {
	...
	$("#section_01 a.href-point").click(function () {
		...
		if (emLen > 0) {
			window.location.href = "/WeixinService/business/query/fundProvinceInfoNew.shtml" + param;
		} else {
...
function loadProvinceInfo() {
	...
	$("#section_02 a.href-point").click(function () {
		...
		var emLen = $(this).find("em").length;
		if (emLen > 0) {
			window.location.href = "/WeixinService/business/query/fundCityInfoNew.shtml" + param;
		} else {
			window.location.href = "/WeixinService/business/query/fundModRiskHouseAddrNew.shtml" + param;
		}
	})
...
function goFundProfessionInfoView() {
	...
	redirectUrl('/WeixinService/business/query/fundProfessionInfoNew.shtml' + param)
};
...
function goFundNationInfoView() {
	...
	redirectUrl('/WeixinService/business/query/fundNationInfoNew.shtml' + param);
};
...
function goFundTaxResidentTypeView() {
	...
	redirectUrl('/WeixinService/business/query/fundTaxResidentTypeNew.shtml' + param);
}
...
function goFundTaxNationInfoView(nationType, targetId) {
	...
	var param = getAllQueryString() + "&nationType=" + nationType + "&targetId=" + targetId + "&taxResidentData=" + encodeURI(taxResidentData)
		+ "&eventId=" + eventId + "&pageSource=" + pageSource;
	redirectUrl('/WeixinService/business/query/fundTaxNationInfoNew.shtml' + param);
};
...
function goFundTaxNotCodeCauseView(targetId) {
	...
	var param = getAllQueryString() + "&targetId=" + targetId + "&taxResidentData=" + encodeURI(taxResidentData)
		+ "&eventId=" + eventId + "&pageSource=" + pageSource;
	redirectUrl('/WeixinService/business/query/fundTaxNotCodeCauseNew.shtml' + param);
}
...
function saveUserTaxInfoToAddrView() {
	...
	if ("product" == sourceType) {
		...
		redirectUrl('/WeixinService/business/user/updateTaxResidentInfoNew.shtml' + param);
	}
}
	...
	$.ajax({
		...
		success: function (data) {
			if (data.returnCode != null && data.returnCode == "0000") {
				if(data.birthDate == "" && data.vocCodeNM == "" && data.nationNM =="" && data.provinceNM == "" && data.addr =="" && data.taxResidentTypeNm ==""){
