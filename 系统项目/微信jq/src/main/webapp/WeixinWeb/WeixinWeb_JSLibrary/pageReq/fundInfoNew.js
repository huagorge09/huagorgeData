...
function goToFAQ() {
	goToURL("/WeixinService/business/query/FAQNew.shtml?fundId="+$("#fundId").val()+"&period="+$("#period").val());
}
function closeCover(){
	$("#lianghua").hide();
}
function goToBuy() {
	...
	goToURL("/WeixinService/business/pay/buyMoneyNew.shtml?fundId=" + fundid+"&period="+period);
}
...
function goToElement(){
	redirectUrl("/WeixinService/business/query/fundElementNew.shtml?orally=fund&fundId="+$("#fundId").val()+"&period="+$("#period").val());
}
/**
 * 财富宝续存中的购买订单列表
 */
function goToRedeemList(){
	//redirectUrl("/WeixinService/business/query/orderListNew.shtml?fundId="+$("#fundId").val()+"&period="+$("#period").val()+"&applyst=G");
	redirectUrl("/WeixinService/business/query/orderListNew.shtml?fundId="+$("#fundId").val()+"&period="+"&applyst=G");
	//redirectUrl("/WeixinService/business/query/orderListNew.shtml?fundId="+$("#fundId").val()+"&period=1"+"&applyst=G");
}
 ...
 */
function goToOldNet(){
	redirectUrl("/WeixinService/business/query/oldNetListNew.shtml?fundId="+$("#fundId").val());
}
/*查询产品净值(曲线图)*/
		...
		success:function(res){
		     if(res.returnCode=="0000"){
		    	if(res.buySatte=="Y"){
		    		...
		    		$(".redeemGo a").attr("href","/WeixinService/business/query/userRedeemNew.shtml?fundId="+fundId+"&period="+period+"&tradeacco="+tradeacco);
				...
				success:function(data){
					...
					desc="招商体系，值得信赖";  //描述
				 	if(eventId=='event_fundListTemplateMsgGuide' && pageSource){ 
				 		link=host+"/WeixinService/weixinLogin/registerNew.shtml?invitation="+data.invitationCode+"&fundId="+param.fundId+"&period="+param.period
				 			+'&eventId=event_shareTemplateMsg'+'&pageSource='+pageSource+'&userid='+userId  //转发链接
				 	}else{
				 		link=host+"/WeixinService/weixinLogin/registerNew.shtml?invitation="+data.invitationCode+"&fundId="+param.fundId+"&period="+param.period  //转发链接
				 	}
				}
			});
		}else{
			link=host+"/WeixinService/weixinLogin/registerNew.shtml?fundId="+param.fundId+"&period="+param.period  //转发链接
		}
	}else{
		$(".invitation").hide()
	}
