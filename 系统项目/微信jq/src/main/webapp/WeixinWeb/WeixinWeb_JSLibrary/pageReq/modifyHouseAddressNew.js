﻿function queryAddressInfo() {
	$("#titleBack").click(function(){
		$('#titleBack').attr("href","javascript:redirectUrl('/WeixinService/business/user/userInfoNew.shtml')");
	});
...
function saveAddress(){
	...
	if(area == region && detailAddress == address){
		setTimeout(function(){window.location.href="/WeixinService/business/user/userInfoNew.shtml";},3000);
	}else{
		$.ajax({
	        async: !1,
...
function loadNationInfo(){
	...
	$("#section_01 a.href-point").click(function(){
		...
		if(emLen > 0){
			window.location.href="/WeixinService/business/user/provinceInfoNew.shtml?nation="+val+"&nationNM="+encodeURI(text);
		}else{
			window.location.href="/WeixinService/business/user/modifyHouseAddressNew.shtml?nation="+val+"&nationNM="+encodeURI(text);
		}
	})
};
...
function loadProvinceInfo() {
	...
	$("#section_02 a.href-point").click(function(){
		...
		var emLen = $(this).find("em").length;
		if(emLen > 0){
			window.location.href="/WeixinService/business/user/cityInfoNew.shtml?nation="+nation+"&nationNM="+encodeURI(nationNM)+"&province="+val+"&provinceNM="+encodeURI(text);
		}else{
			window.location.href="/WeixinService/business/user/modifyHouseAddressNew.shtml?nation="+nation+"&nationNM="+encodeURI(nationNM)+"&province="+val+"&provinceNM="+encodeURI(text);
		}
	})
};
...
function loadCityInfo() {
	...
	$("#section_03 a.href-point").click(function(){
		var val = $(this).data("value");
		var text = $(this).data("text");
		window.location.href="/WeixinService/business/user/modifyHouseAddressNew.shtml?nation="+nation+"&nationNM="+encodeURI(nationNM)+"&province="+province+"&provinceNM="+encodeURI(provinceNM)+"&city="+val+"&cityNM="+encodeURI(text);
	});
};