...
$(document).ready(function(e){
    ...
    getRandomCode();
    myScroll.refresh();
    getUserRequest("register-input");/* 此处subPath为页面内行为 */
//  mgm开关
    mgmInit()  
});
...
function checkRandomCode(){
    ...
    var confirmPassWord = $("#confirmPassWord").val();
    var randomCode = $("#randomCode").val();
    var inventer_mobile= $("#inventer_mobile").val();
    ...
    $.ajax({
        ...
        success : function (data){
            if(data.returnCode=="0000"){
                ...
                $("#section_01").hide();
                $("#section_02").show();
                getUserRequest("register-input-review");/* 此处subPath为页面内行为 */
...
function register(){
    ...
    if(mobile == null || mobile == "" || !isMobile(mobile)){
        ...
        var entrance = getUrlParameter("entrance");
        $.ajax({
            async: false,
            url: "/WeixinService/user/weixinRegister.xhtml",
            data: {
                "mobile": mobile,
                "passWord": passWord,
                "channel": "WXIE",
                    ...
                    if(""!=entrance&&entrance!=null){
                        ...
                        $.ajax({
                            ...
                            success : function (data){
                                if(data.returnCode=="0000"){
                                    /*领取成功,电影券,以逗号隔开*/
                                    redirectUrl("/WeixinService/activity/fartherDayActivity/fartherDaySuccessNew.shtml");
                                }else if(data.returnCode=="0010"){
                                    /*奖品已领完*/
                                    redirectUrl("/WeixinService/activity/fartherDayActivity/fartherDayBroughtOutNew.shtml");
                                }else if(data.returnCode=="0020"){
                                    /*已经领取过,电影券,以逗号隔开*/
                                    var strs = new Array(); /* 定义一数组 */
                                    strs = data.returnMsg.split(","); /* 字符分割 */
                                    redirectUrl("/WeixinService/activity/fartherDayActivity/fartherDayRepetitionNew.shtml?ticket1=" + strs[0] + "&ticket2=" + strs[1]);
                                    ...
                                    /*跳转到注册页*/
                                    redirectUrl("/WeixinService/weixinLogin/registerNew.shtml?entrance=fartherDayActivity");
                                }else{
                                    errorRemark("网络繁忙，请稍后再试!");
                                }
                            }
                        ...
                        $("#section_02").hide();
                        $("#section_03").show();
                        getUserRequest("register-success");/* 此处subPath为页面内行为 */
                    }
...
function mgmInit(){
	...
	if(param=="1"){  //开启MGM验证
		...
		if(!!invitFlag.invitation){  //该用户是通过邀请链接
			...
			if(invitFlag.fundId){
				            localStorage.setItem("mgmFundId",invitFlag.fundId)
			localStorage.setItem("mgmPeriod",invitFlag.period)
			}
