...
function commonMenth(i, item, Number) {
	var listhtml = "";/* 产品列表 */
	if(item.typeId=="0210"){
		listhtml += '<div class="pro-box type0210" onclick="redirectUrl(\'/WeixinService/business/query/fundInfo.shtml?fundId=' + item.fundId + '&period='+item.period+'\')">';
	}else{
		listhtml += '<div class="pro-box" onclick="redirectUrl(\'/WeixinService/business/query/fundInfo.shtml?fundId=' + item.fundId + '&period='+item.period+'\')">';
	}
...
function checkUserBaseInfoIsExist(type){
	...
	if(!!resultData && resultData.flag){
		...
		if(type){
			...
			$('#goRisk,#reRisk').click(function(){
				addCookie('riskUrl',location.href);
				redirectUrl("/WeixinService/business/user/riskLevel.shtml?viewType=prodcut&eventId="+eventId+"&pageSource="+pageSource);
			});
		}/*else if(type == 2){
			//测评过期
			$('#reRisk').click(function(){
