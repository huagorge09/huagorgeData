		...
		success: function (data) {
			if (data.returnCode != null && data.returnCode == "0000") {
				...
				if (userType != null && userType == 30) {
					...
					$("#idNo").html(data.idNo);
					//  身份证有效期
					$('#idTImePoint').attr("href", "javascript:redirectUrl('/WeixinService/business/user/updateIdCardIndate.shtml')");
					if (isSetTradePassword != null && isSetTradePassword == "Y") {
						$("#isSetTradePassword").html("已设置").addClass("act");
						$("#isSetTradePassword").parent().attr("href", "javascript:redirectUrl('/WeixinService/business/user/modifyTPassword.shtml')");
						$("#mobile").parent().attr("href", "javascript:redirectUrl('/WeixinService/business/user/modifyRegMobile.shtml')");
					} else {
						$("#isSetTradePassword").html("未设置");
						$("#isSetTradePassword").parent().attr("href", "javascript:redirectUrl('/WeixinService/business/user/setTPassword.shtml')");
					}
				}
				...
				//20180315 去掉个人信息实名验证
				/*else{
					$("#userName,#idNo").parent().attr("onclick","redirectUrl('/WeixinService/business/bank/bankAuth.shtml')");
					$("#mobile").parent().attr("href","javascript:redirectUrl('/WeixinService/business/bank/bankAuth.shtml')");
					$("#isSetTradePassword").html("未设置");
					$("#isSetTradePassword").parent().attr("href","javascript:redirectUrl('/WeixinService/business/bank/bankAuth.shtml')");
				}*/
				...
				$("#homeAddress").parent().attr("href", "javascript:redirectUrl('/WeixinService/business/user/modifyHouseAddress.shtml?region=" + address + "&address=" + data.addr + "')");
				/*if(data.addr != "" && data.addr != null){
					address+="-"+data.addr;
				}*/
				if (address != null && address != "") {
				...
				$("#goRiskBtn").click(function () {
					window.location.href = "/WeixinService/business/user/riskLevel.shtml?invprtp=1";
				})

				//新增字段
				$("#birthDateTxt").parent().attr("href", "javascript:redirectUrl('/WeixinService/business/user/updateBirthDate.shtml?birthDate=" + data.birthDate + "')");
				if (data.birthDate != "" && data.birthDate != null) {
					$("#birthDateTxt").html(data.birthDateNm);
					$("#birthDateTxt").removeClass("icon");
				}
				//新增字段
				$("#taxResidentTypeText").parent().attr("href", "javascript:redirectUrl('/WeixinService/business/user/updateTaxResidentInfo.shtml?taxResidentType=" + data.taxResidentType + "&taxResidentTypeNM=" + data.taxResidentTypeNm + "')");
...
function goRiskPage(birthDate) {
	window.location.href = "/WeixinService/business/user/riskLevel.shtml?birthDate=" + birthDate + "&pageSource=wx_personalInformationId&eventId=event_wx_personRiskId";
}
...
function invtpApplyTask(invprtp, appst, invprtpScore) {
	...
	if (invprtp == "" || invprtp == null) {
		...
		//跳转至申请专业投资者页面
		window.location.href = "/WeixinService/business/user/commonInvprExplain.shtml";
	} else if (invprtp == 1 || invprtp == '1') {
		if (appst == 'N' || appst == 'I') {
			//invprtpStr = '普通投资者（专业投资者申请中）';
			window.location.href = "/WeixinService/business/user/materialReviewPage.shtml";
		} else if (appst == 'C') {
			$("#hint_01").show();
		} else {
			//跳转至申请专业投资者页面
		...
		if (invprtpScore < 60 || invprtpScore == null || invprtpScore == "") {
			...
			window.location.href = "/WeixinService/business/user/materialReviewPage.shtml?pic=2";
		} else {
			//invprtpStr = "专业投资者（申请转为普通投资者）";
			window.location.href = "/WeixinService/business/user/specialtyInvprExplain.shtml";
		}
	}
}
...
function openRiskPage() {
	...
	if ("0" == invprtp) {
		...
		window.location.href = "/WeixinService/business/user/riskLevel.shtml?invprtp=" + encodeURI(invprtp) + "&appst=" + encodeURI(appst);
	}
}
