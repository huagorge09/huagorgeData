...
$(document).ready(function(e) {
	...
	queryNewsArticle();
	loaded();
	getUserRequest("news-list");/* 此处subPath为页面内行为 */
});
...
function queryNewsArticle() {
	$.ajax({
		async : true,
		url : "/WeixinService/article/queryNewsArticle.xhtml",
		data : {
			"page" : page
		},
		dataType : "json",
				...
				if (list != null && list.length > 0) {
					...
					$.each(list, function(i, item) {
						if (item.readFlag != null && item.readFlag == "0") {
							htmls += "<div class='box-news news'";
							htmls += " onclick='articleDetail(\"" + item.articleId + "\")' ><dl>";
						} else {
							htmls += "<div class='box-news news act'";
...
function articleDetail(articleId) {
	...
	redirectUrl("/WeixinService/article/articleContentNew.shtml?articleId=" + articleId);
}
/* 查询消息详情 不保存已读 */
function articleDetailD(articleId) {
	redirectUrl("/WeixinService/article/articleContentNew.shtml?type=1&articleId=" + articleId);
}
/* 信批已读保存 */
