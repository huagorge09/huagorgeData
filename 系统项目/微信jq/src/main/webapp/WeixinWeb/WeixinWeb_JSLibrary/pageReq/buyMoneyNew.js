...
function toMyaccount(){
	...
	redirectUrl('/WeixinService/business/user/riskLevelNew.shtml?eventId=event_wx001_reassessId&pageSource=wx_confirmOrderId');
}
		...
		success : function(data) {
			...
			if (data.returnCode == "0000") {
				if(data.userType == null || data.userType != "30"){
					...
					window.setTimeout("redirectUrl('/WeixinService/business/bank/bankAuthNew.shtml?pageSource=wx_confirmOrderId')", 1000);
...
function goToAddBank(){
	...
	window.location.href="/WeixinService/business/bank/addBankNew.shtml?fundId="+param+"&period="+period;
}
		...
		success : function(data) {
			if (data.returnCode != null && data.returnCode == "0000") {
				if(data.taxResidentType == ""){
                                        setTimeout(function(){
				    	     window.location.href="/WeixinService/business/user/updateTaxResidentInfoNew.shtml"
				        },2000)
