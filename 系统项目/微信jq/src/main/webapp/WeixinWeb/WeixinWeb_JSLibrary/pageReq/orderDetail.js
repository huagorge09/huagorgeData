        		...
        		if(data.dto == null &&data.tradeList==null){
    				...
    				if(typeId!=null&&(typeId=='0100'||typeId=='0300')||typeId=='0400'){/* 固收产品或者浮动收益 0400为7天14天理财*/
    					...
    					if(orderType!=null&&orderType=='1'){/* 未支付 */ 
                            ...
                            htmls += "<a href='javascript:redirectUrl(\"/WeixinService/business/query/modifyMoney.shtml?serialno="+dto.serialno+"\")' class='fr'>修改</a></li>";
    					...
    					if(daysBetween(currentWorkdate, maturityDate)==0){
    						...
    						renewUpdate='<a  href="javascript:redirectUrl(\'/WeixinService/business/query/modifyPaymentWay.shtml?serialno='+dto.serialno+'&renew='+dto.renew+'\')" class="fr">修改</a></li>'
    					}
    					...
    					if(orderType!=null&&orderType=='1'){/* 未支付 */ 
                            ...
                            if(renew=='N'){
								...
								if(typeId =='0110') {
                            		...
                            		htmls += "<span class='order-main-point1'>续投成功，本金与收益已自动续投下一期产品，<a href='/WeixinService/business/query/orderDetail.shtml?serialno="+serialno+"&period="+period+"' >查看续投订单";
                            	}
							}
                            htmls+="</span>"
    					}else if(orderType!=null&&orderType=='6'){/* 已失效 */
					...
					if (typeof fundInfo.elementList != 'undefined') {
    					...
    					$("#goToFAQ").attr("onclick","redirectUrl('/WeixinService/business/query/FAQ.shtml?fundId="+fundInfo.fundId+"&period="+fundInfo.period+"');");
...
function goToElement(){
	...
	redirectUrl("/WeixinService/business/query/fundElement.shtml?orally=order&period="+$("#period").val()+"&fundId="+$("#fundId").val()+"&serialno="+serialno);
}
function cancleTips(){
	$("#cancelOrderDiv").show();
}
	...
	$.ajax({
        ...
        success : function (data){
			if(data.returnCode != null && data.returnCode == "0000"){
				closeTips('cancelOrderDiv');
				errorRemark("取消订单成功");
				setTimeout('redirectUrl("/WeixinService/business/query/orderList.shtml")',2000);
			}else{
				closeTips('cancelOrderDiv');
				errorRemark("取消订单失败");
			}
...
function showOrder(orderType){
	...
	htmls3 += "<a href='javascript:redirectUrl(\"/WeixinService/business/query/modifyBankcard.shtml?serialno="+serialno+"\")' class='fr'>更换</a></li>";
	...
	htmls3 += "<a href='javascript:redirectUrl(\"/WeixinService/business/query/modifyMoney.shtml?serialno="+serialno+"\")' class='fr'>修改</a></li>";
...
function comfirmOrder(serialno){
	redirectUrl("/WeixinService/business/pay/confirmBuy.shtml?serialno="+serialno);
}
		...
		success : function(data) {
			if (data.returnCode == '0000') {
				...
				if (data.fundContractDto != null) {
					...
					htmls = "产品合同：<a class='contact' href='javascript:goToContract(\"/WeixinService/business/query/fundContract.shtml?templet=" + fundId + "&period="+period+"\")'>《" + fileName + "》</a><br/>";
				} else {
 ...
 */
function goToOldNet(){
	redirectUrl("/WeixinService/business/query/oldNetList.shtml?fundId="+$("#fundId").val()+"&isSub="+$("#isSubPrdAppraisement").val()+"&outerId="+$("#outerId").val());
}
/*查询产品净值(曲线图)*/
		...
		success:function(res){
		     if(res.returnCode=="0000"){
		    	if(res.buySatte=="Y"){
		    		...
		    		$(".redeemGo a").attr("href","/WeixinService/business/query/userRedeem.shtml?fundId="+fundId+"&period="+period);
