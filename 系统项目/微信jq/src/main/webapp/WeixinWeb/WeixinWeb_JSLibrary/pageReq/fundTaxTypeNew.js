...
function clickTaxResidentType(element){
	...
	if("1" == type){
		...
		redirectUrl('/WeixinService/business/query/fundTaxInfoNew.shtml' + param);
	}
}
...
function clickTaxNation(element){
	...
	if("1" == type){
		...
		redirectUrl('/WeixinService/business/query/fundTaxAreaInfoNew.shtml' + param);
	}
}
...
function getAllQueryString(){
	...
	param += "nation="+nation+"&nationNM="+encodeURI(nationNM)+"&province="+province+"&provinceNM="+encodeURI(provinceNM)
					...
					+"&sourceType="+sourceType+"&viewType="+viewType+"&otherVocation="+otherVocation+"&eventId="+eventId+"&pageSource="+pageSource
					;
