...
$(function() {
	if(getUrlSearchParams("eventId") && getUrlSearchParams("pageSource")){
		localStorage.setItem("eventId", getUrlSearchParams("eventId"));
		localStorage.setItem("pageSource", getUrlSearchParams("pageSource"));
	}
	silentAccredit()
	queryUserToRealName()
	queryUserInfo()
...
function silentAccredit() {
	if (!userid) { //判断用户有没有静默授权 wx	
		...
		location.href = host + "/auth/proxy-silent.html?scope=snsapi_base&target_url=" + host +
			"/WeixinService/business/mgm/mgmPromotionActNew.shtml";
	} else {
		
		// 在localstorage里存userid
		eventId=getUrlSearchParams("eventId") ? getUrlSearchParams("eventId") : localStorage.getItem("eventId")
		pageSource=getUrlSearchParams("pageSource") ? getUrlSearchParams("pageSource") : localStorage.getItem("pageSource")
		...
		success: function(data) {
			if (data.success) {
				...
				if (data.resp.returnCode == "N") { //如果用户未绑定,即未登录
					location.href = "/WeixinService/weixinLogin/loginNew.shtml?pageSource=" + pageId;
					return;
				}else{
					//  如果用户未实名
...
$('.toRealBtn').click(function() {
	window.location.href = '/WeixinService/business/bank/bankAuthNew.shtml';
})
...
function dataRecord(customerName,customerMobile) {
	...
	var eventId = 'event_mgmPromoSubmit';
	//  顾问id
...
function pageDataRecord() {
	...
	var eventIdTemp = eventId==null?'wx_mgm_actId':eventId;
...
function sharePageStyle() {
	link = host + '/WeixinService/business/mgm/mgmPromotionActNew.shtml';
