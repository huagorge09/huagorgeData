		...
		success : function(data) {
			if (data.returnCode == '0000') {
				...
				if (data.fundContractDto != null) {
					...
					htmls = "产品合同：<a class='contact' href='javascript:goToContract(\"/WeixinService/business/query/fundContract.shtml?templet=" + fundId  + "&period="+period+"\")'>《" + data.fundContractDto.templateName + "》</a><br/>";
				} else {
...
function goToElement(){
	var serialno = getUrlParameter("serialno");
	var channleNo = getUrlParameter("channleNo");
	redirectUrl("/WeixinService/business/query/fundElement.shtml?orally=orderOther&period="+$("#period").val()+"&fundId="+$("#fundId").val()+"&serialno="+serialno+"&channleNo="+channleNo);
}
/**
 * 历史净值页面入口
 */
