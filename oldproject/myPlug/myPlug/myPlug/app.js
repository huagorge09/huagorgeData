...
;(function(global, factory) {
    ...
    var _app = {
        ...
        client : {              //系统设备详细解析信息  tips: 根据浏览器代理自动判断赋值
            android : 0
            ,ios : 0
        },
        ...
        init : function (options) {
            ...
            _setBase();     //基础配置
            _loadJSON2();   //加载json2库
            _loadFastclick();   //加载fastclick库
            //rem设置、初始化百度统计
           // self.setRem(self.opt).logCookie().initBaidu();
            _setFlag();     //设置配置标识
            _bindError();   //捕获js等报错信息，打印日志
            self.log(decodeURIComponent(self.url), '页面地址');
            //返回当前对象，便于链式操作
            return self;
        },
            ...
            if(self.isPrint){//是否将日志信息打印添加到页面上显示
                setTimeout(function(){
                    try{
                        ...
                        doc.body.appendChild && doc.body.appendChild(newDom);
                        ...
                        doc.body.appendChild && doc.body.appendChild(newDom2);
        ...
        toString : function (obj) {
            var res = obj;
            try{
                win.JSON && JSON.stringify && ( res=JSON.stringify(obj) );
            }catch(e){}
            return res;
        },
        ...
        toObject : function (str) {
            var res = str;
            try{
                win.JSON && JSON.stringify && ( res=JSON.parse(str) );
            }catch(e){}
            return res;
        },
 ...
 */
        ...
        /**
         * [getCookieObject 遍历cookie，生成键值对对象]
         */
        getCookieObject : function(){
            var aCookie = document.cookie.split(";");
            if(aCookie.length==1 && !aCookie[0]){
                return null;
            }
            var re = {};
            for (var i = 0; i < aCookie.length; i++) {
                var item = aCookie[i].replace(/\s+/g, "").split("=");
                re[ item[0] ] = item[1];
            }
            return re;
        },
        ...
        /**
         * [logCookie 初始化每隔一段时间打印一次Cookie]
         */
        logCookie : function(){
            var self = this,
                oCookie = self.getCookieObject(),//获取Cookie对象
                count = 1,//初始化打印次数
                time = 5000,//配置多久打印一次，这里5秒更新一次
                max = 1;//配置最大打印次数，这里1次，其他情况要打印请自行打印
            var inId = setInterval(function(){
                count >= max && clearInterval(inId);//大于指定次数不再打印
                self.log(oCookie, 'cookie数据'+count);//日志打印
                count++;//次数增加
            }, time);
            return self;
        },
        ...
        /**
         * [location 页面跳转，此跳转可以将调试的参数往下传递，如果都是我们一整套架构的页面，可以用此进行页面跳转]
         */
        location : function(url, replaceFlag){
            var self = this,
                arr = [];
            self.isMock && arr.push('isMock=true');
            self.islog && arr.push('islog=true');
            self.isAlert && arr.push('isAlert=true');
            self.isPrint && arr.push('isPrint=true');
            self.isGlobal && arr.push('isGlobal=true');
            if(arr.length>0){
        ...
        /**
         * [reload 页面刷新,强制加版本号，解决安卓上直接调window.reload不生效的问题]
         */
        reload : function(){
//            var url = location.href,
//                pre = url.indexOf('?')==-1 ? '?_t=' : '&_t=',
//                goUrl = url+pre+(+new Date());

            App.params['t'] = ((new Date()).getTime());
            var _params = App.params;
            var str="";
            //没有没法依赖 $.param
            for (x in _params)
            {
                str+=encodeURIComponent(x)+'='+ encodeURIComponent(_params[x])+'&';
            }

            var goUrl = location.href + '?' + str;
    ...
    function _setDevice() {
        ...
        if(win.navigator){
            ...
            var lDev = self.device.toLowerCase(),
                ...
                weixinReg = lDev.match(/micromessenger\/([\d\.]+)/i),
                android = lDev.match(/(android);?[\s\/]+([\d.]+)?/),
                ios = lDev.match(/cpu (?:iphone )?os (\d+_\d+)/);
            if( mReg ){
                self.isMobile = true;
            }
            if( ieReg ){
            ...
            if(android){
                self.client.android = android[2];
            }
            if(ios){
                self.client.ios = ios[1].replace(/_/g, '.');
            }
    ...
    function _setBase() {
        ...
        if(self.evn == 'dev'){
            ...
            //测试cdn域名
            if(self.sysDir!='/nctest-eb-web/'){//预演环境不需要cdn
                self.cdnUrl = '//stest.sinosafe.com.cn' + self.sysDir;
            }
            //
            ...
            //后续生产改成具体的cdn域名
            self.cdnUrl = '//s.sinosafe.com.cn' + self.sysDir;
        }
    ...
    /**
     * [_loadFastclick 动态加载fastclick库 移动端click事件优化]
     */
    function _loadFastclick() {
        var self = _app;
        if(self.isMobile){
            //  self.loadJs(self.cdnUrl + 'libs/fastclick/fastclick.js', null, false, false);
            //2017-01-03 修复	self.loadJs加载不能绑定DOMContentLoaded事件
            document.write( '<script src="' + self.cdnUrl + '/libs/fastclick/fastclick.js" type="text/javascript"></script>' ) ;
        }
    };
