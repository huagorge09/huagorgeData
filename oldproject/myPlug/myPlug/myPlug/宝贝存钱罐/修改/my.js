...
var page = function () {
    ...
    function initData(){
        $.ajax({
            ...
            dataType: 'json',
            async:false,
            data:"viewId="+viewId+"&keyWord="+keyWord+"&isHasPhoto="+isHasPhoto+"&isVoice="+isVoice+"&isJH="+isJH+"&beginDate="+beginDate,
            error: function(){
                $(this).alertFnOne({
                    title: '温馨提示',
                    content: '查询数据异常，请重试'
    ...
    function appendHtml(resultData){
        ...
        if(sumLength > 0){
            ...
            $(".notContain").show().html('<a class="handle" href="javascript:void()">没有找到相应足迹,<span>重新筛选</span></a>');
        }
    ...
    function footprintOperation() {
        ...
        $('.dayItem .moreWrap').click(function(e) {
            e.stopPropagation();
        });
