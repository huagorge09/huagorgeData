var common = {
    ...
    previewPicture: function (Object) {
        ...
        for(var k = 0;k < urlList.length;k++){
            srcList.push(urlList[k].src);
        }
        var index = srcList.indexOf(currentUrl) == -1 ? 0 : srcList.indexOf(currentUrl);
        //添加swiper元素
        ...
        for (var i = 0; i < urlList.length; i++) {
            html += '<div class="swiper-slide"><img src="' + urlList[i].src + '" picNo="'+urlList[i].picNo+'"></div>';
        }
        ...
        $("#previewPicture-delete").on("click", function (e) {
            ...
            common.affirmHintBox({
                ...
                callback: function (res) {
                    if (res === "confirm") {
                        Object.removeImgIds.push(urlList[previewPicture.activeIndex].picNo);
    ...
    getUrlSearchParams:function (_name){
        var name,value='';
        var str=window.location.href.split('?')[2];
        var num=str.indexOf("?");
...
(function (window) {
    function PictureMove(ele) {
        var that = this;
        var sx,sy,mx,my,imgW,imgH,wrap,wrapW,wrapH,imgL,imgT,wrapL,wrapT;
        ele.on("click",function () {
            wrap = $(this).parent();
            wrap.siblings().removeClass("imgWrapShadow");
            if(wrap.hasClass("imgWrapShadow")){
                wrap.removeClass("imgWrapShadow");
            }else {
                wrap.addClass("imgWrapShadow");
            }
        });
        ele.on("touchstart",function (e) {
            ...
            wrapL = wrap.position().left;
            wrapT = wrap.position().top;
        }).on("touchmove",function (e) {
            mx = e.changedTouches[0].pageX;
            my = e.changedTouches[0].pageY;
            if(wrap.hasClass("imgWrapShadow")){
                that.handWrap(wrap,mx-sx,my-sy);
            }else {
                that.handleImg($(this),mx-sx,my-sy);
            }
            e.preventDefault();
        }).on("touchend",function () {
            that.wrapMoveEnd();
        });
        this.handleImg = function (that,x,y) {
        ...
        this.handWrap = function (that,x,y) {
            var dx = Math.abs(that.parent().width()-that.width());
            var dy = Math.abs(that.parent().height()-that.height());
            var left = wrapL + x;
            var top = wrapT + y;
            if(left > dx){
                left = dx;
        ...
        this.wrapMoveEnd = function () {
            var x = wrap.position().left + wrap.innerWidth()/2;
            var y = wrap.position().top + wrap.innerHeight()/2;
            var replaceWrap;
            ele.each(function () {
            ...
            if(replaceWrap){
                wrap.css({
                    "left" : replaceWrap.position().left,
                    "top" : replaceWrap.position().top,
                    "width" : replaceWrap.innerWidth(),
                    "height" : replaceWrap.innerHeight()
                }).children("img").css({
                    "left":0,
                    "top":0
                });
...
(function () {
    ...
    function CompositePicture(hasPicture,callback) {
        $("#generatePhotosBox").show();
        $("body").append("<div class='mask' id='generatePhotosBox_mask'></div>");
        var that = this;
    ...
    CompositePicture.prototype = {
        setCanvas : function (cvs,contain) {
            ...
            this.ctx.rect(0,0,this.cvs.width,cvs.height);
        },
        drawHasPicture : function () {
            ...
            function drawPictureList() {
                if(i<pictureList.length){
                    ...
                    img.onload = function () {
                        var at = parseInt((i+1)/pictureList.length*100);
                        $("#generatePhotosBox .photosWrap img").css("top",at+"%");
                        $("#generatePhotosBox .per").text(at+"%");
        ...
        drawNotPicture : function () {
            $("#generatePhotosBox .photosWrap img").css("top","100%");
            $("#generatePhotosBox .per").text("100%");
            this.drawCode();
            this.drawJar();
        ...
        drawFooter:function () {
            ...
            image.onload = function () {
                ...
                $("#generatePhotosBox").hide();
                $("#generatePhotosBox_mask").remove();
                $("#generatePhotosBox .photosWrap img").css("top","0%");
                $("#generatePhotosBox .per").text("0%");
                that.callback(that.cvs);
            }
        }
    };
