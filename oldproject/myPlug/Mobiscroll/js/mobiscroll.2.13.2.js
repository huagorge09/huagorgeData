    ...
    ms.classes.Widget = function (el, settings, inherit) {
        ...
        */
        that.select = function () {
            ...
            //console.log($("#money_dummy").val())
            //console.log($("#total_time_dummy").val())
            //console.log($("#total_time_dummy").val())
            ...
            if(that.val.split("-").length == 3){
                var time = new Date().getFullYear() - +$("#insBirth").val().split("-")[0];
                if(new Date().getMonth() + 1 <  +$("#insBirth").val().split("-")[1] || (new Date().getMonth() + 1 ==  +$("#insBirth").val().split("-")[1] && new Date().getDate() < +$("#insBirth").val().split("-")[2])){
                    time--;
                }
                console.log(time)
            }
            var seventy = $(".seventy");
            if(time>=46 && seventy.hasClass("active")){
                $(".twenty").hide();
                if($(".twenty").hide()){
                    $(".twenty").removeClass("active").siblings().addClass("active");
                    colYear="10";
                    getProCalInfo();
                }
            }else{
            ...
            if(time>=41){
                    ...
                    $(".age_o").hide();
                    if($(".selecTion#faceAmount li.active").index() >= 3){
                        $(".selecTion#faceAmount li.active").removeClass("active");
                        $(".selecTion#faceAmount li").eq(1).addClass("active");
                        amount="20";
                        getProCalInfo();
                    }
    ...
    $.mobiscroll.presets.scroller.select = function (inst) {
        ...
        function setVal(v, fill, change) {
            ...
            if (multiple) {
                ...
                input.val(v);
                var str = $('option[value="' + option + '"]').parent().attr("label");
                $("#names_dummy").val(str + " " + v);
